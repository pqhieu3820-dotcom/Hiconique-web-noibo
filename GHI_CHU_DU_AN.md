...
**Đang dở dang: nút "Báo cáo công việc" (CEO/Manager, trang Dự án) — code đã viết xong (HTML+CSS+JS) nhưng CHƯA test trên trình duyệt, CHƯA commit/push.** Xem chi tiết field/hàm liên quan ở mục "Trạng thái hiện tại — 2026-09-11 (c)" ngay dưới — người dùng yêu cầu vì bảng "Tiến độ hôm nay"/"Lịch hôm nay" cũ ở trang Tasks (`tasks-manager.html`) hoá ra là mockup tĩnh 100% (không có dòng JS nào wire dữ liệu thật vào, xem comment trong file) — CHƯA sửa/gỡ phần mockup đó, chỉ mới xây tính năng báo cáo mới thay thế nhu cầu thực sự của người dùng.
...
## Trạng thái hiện tại (cập nhật lần cuối: 2026-09-11 (c) — nút "Báo cáo công việc" CEO/Manager (ĐANG DỞ DANG, chưa test/chưa push) + map lại 3 cột phụ Thành viên, đọc kỹ mục này trước)
...
2. **Nút "Báo cáo công việc" (CEO/Manager) ở trang Dự án — ĐANG DỞ DANG, CHƯA TEST, CHƯA COMMIT/PUSH.** Theo yêu cầu người dùng: xem tiến độ dự án + tiến độ từng người (tổng hoặc lọc 1 người) + nhật ký tiến độ hàng ngày từng task, theo thời gian thực. Người dùng còn báo bảng "Tiến độ hôm nay"/"Tuần này"/"Lịch hôm nay" cũ ở góc phải trang `tasks-manager.html` "không hoạt động" — kiểm tra thấy đây là **mockup HTML tĩnh 100%, chưa từng có JS nào wire dữ liệu thật vào** (số 63%, "5/8 tasks", biểu đồ tuần, 3 sự kiện lịch — toàn bộ đều hardcode sẵn trong `tasks-manager.html`, `task-manager-app.js` không hề động tới các id/class đó). Quyết định: xây tính năng "Báo cáo công việc" MỚI ở trang Dự án đúng theo yêu cầu cụ thể của người dùng thay vì sửa lại bảng mockup cũ đó — **bảng mockup cũ ở `tasks-manager.html` vẫn còn nguyên, CHƯA sửa/gỡ, cần quay lại xử lý sau** (xoá hẳn hoặc thay bằng dữ liệu thật, tuỳ người dùng chọn).
   ...
   - **Việc còn lại trước khi coi là xong**: mở thật trên trình duyệt (đăng nhập CEO/Manager), bấm nút, kiểm tra cả 4 phần render đúng + không lỗi console, thử đổi bộ lọc Dự án/Thành viên, thử tài khoản `member` thường xác nhận nút bị ẩn — rồi mới commit + push.
