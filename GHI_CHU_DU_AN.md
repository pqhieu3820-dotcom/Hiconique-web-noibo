...
- **UI dòng "Vị trí GPS" trong bảng xác thực chấm công** (`timesheet.html`) — nút "Thử lại"/"Cấp quyền vị trí" trước đó trôi nổi cạnh cả khối 3 dòng địa chỉ (chữ dài xuống dòng làm nút lệch giữa trông rất lạc quẻ). Đã gộp nhãn "Vị trí GPS" + nút vào chung 1 dòng đầu (`.ts-verify-head`), nhãn CANH GIỮA trong phần còn lại của hàng (không dính sát trái), nút canh phải — cả hai canh giữa theo chiều dọc với nhau; dòng địa chỉ/thông báo xuống dòng riêng bên dưới full-width. 2 dòng "Mạng Wifi công ty"/"Thiết bị đã đăng ký" (không có nút) giữ nguyên canh trái như cũ.
