...
const Auth = (function() {
  ...
  function showRegisterModal() {
    ...
    var html = `
      <style>
        #authLoginModal .auth-card { padding: 40px; }
        #authLoginModal .auth-grid-2 { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; }
        #authLoginModal input, #authLoginModal select {
          width: 100%; padding: 12px 14px; background: var(--color-bg); border: 1px solid var(--color-border);
          border-radius: 8px; color: var(--color-text); font-size: 0.9375rem; font-family: inherit; outline: none;
          transition: border 0.2s; box-sizing: border-box;
        }
        #authLoginModal input::placeholder { font-size: 0.875rem; }
        @media (max-width: 480px) {
          #authLoginModal .auth-card { padding: 24px 18px; }
          #authLoginModal .auth-grid-2 { grid-template-columns: 1fr; gap: 16px; }
