...
- **Offline-first (chốt 2026-09-16)**: mọi trang PHẢI hoạt động xem-được khi mất mạng (iOS Safari + Android Chrome), và MỌI hàm ghi dữ liệu mới thêm sau này PHẢI tự chặn khi mất mạng — xem `public/js/offline.js` (`Offline.guard('nhãn hành động')` — gọi ở ĐẦU hàm, trả `true` = đã chặn + tự hiện toast, return sớm ngay). 3 lớp:
  1. **App shell offline** — `public/sw.js` (Service Worker) precache toàn bộ HTML/CSS/JS/icon, đăng ký qua `offline.js`. Đổi `CACHE_VERSION` trong `sw.js` mỗi khi thêm/đổi tên file tĩnh mới (thêm trang mới PHẢI thêm vào mảng `PRECACHE_URLS`), không thì trang mới sẽ không mở được lúc mất mạng.
  2. **Banner + mốc giờ dữ liệu** — `offline.js` tự chèn banner cố định đầu trang khi mất mạng, hiện đúng giờ:phút:giây + ngày/tháng/năm của lần đồng bộ THÀNH CÔNG gần nhất (`localStorage['hiconique_last_sync']`, tự cập nhật ở `fetchFromAPI()` trong `task-data.js`). Có mạng lại → tự `Offline` phát hiện (ping định kỳ 15s vào chính Apps Script Web App, KHÔNG chỉ tin `navigator.onLine` vì Safari iOS hay báo sai) → tự `window.location.reload()` sau ~1s để lấy dữ liệu mới, không tự vá UI từng trang.
  3. **Chặn ghi khi mất mạng** — đã chặn ở các hàm dùng chung `add()`/`update()`/`remove()`/`callGSheetsAPI()` (`task-data.js`, hầu hết mọi loại dữ liệu đi qua đây) + các hàm ghi lẻ gọi thẳng API (`addAttendanceLocation`/`updateAttendanceLocation`/`deleteAttendanceLocation`/`addSystemNotificationsBatch`, `Auth.register()` trong `auth.js`, `gsWrite()` trong `hicon-bim.html`/`pricing.html`). Thêm 1 luồng ghi MỚI không đi qua các hàm này (VD gọi `fetch(GSHEETS_CONFIG.API_URL...)` thẳng ở 1 trang khác) → PHẢI tự thêm `if (typeof Offline !== 'undefined' && Offline.guard('...')) return;` ở đầu, không được bỏ qua.
...
## ⏳ VIỆC CÒN TỒN ĐỌNG (đọc mục này đầu tiên — cập nhật 2026-09-16 cuối phiên)

Không có việc gì đang dở dang — mọi thay đổi trong phiên này đều đã deploy (Apps Script phiên bản 65) + xác nhận trên trình duyệt (chờ lệnh push cuối phiên).
