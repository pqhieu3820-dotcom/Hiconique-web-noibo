...
Còn 1 việc dở dang: đã COMMIT LOCAL nhưng **CHƯA PUSH** phần "Làm lại quy trình Task theo 4 cột" (xem mục ngay dưới) — chờ người dùng gõ "push" trước khi đẩy lên GitHub. Apps Script đã redeploy lên **phiên bản 73** (thêm cột `Ghi chú duyệt`/`reviewNote` cho sheet "Tasks" — xem mục ngay dưới).
