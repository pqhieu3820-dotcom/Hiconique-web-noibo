...
- **KHÔNG DÙNG `alert()`/`confirm()` gốc của trình duyệt ở bất kỳ trang nào** (Chốt cứng 2026-09-15 sau khi cả 2 hàm này làm TREO CỨNG tab thật khi test bằng Claude in Chrome — dialog native chặn hẳn main thread, công cụ automation không click/chụp màn hình/chạy JS được nữa, phải đóng hẳn tab mới thoát ra được.) Dùng `showConfirmDialog(title, onConfirm, confirmLabel)` (2 nút, tự dựng bằng `.ts-confirm-overlay`) thay `confirm()`, và `showToast(message, ok)` (banner tự biến mất, không chặn) thay `alert()` — cả 2 hàm mẫu đã có sẵn trong `timesheet.html`, copy pattern y hệt sang trang khác nếu cần.
- **KHÔNG gọi API ghi (add/update) nhiều lần SONG SONG trong 1 vòng lặp từ client** (VD gửi thông báo tới nhiều người cùng lúc). Mỗi lần gọi API là 1 lần thực thi Apps Script riêng — nhiều lệnh chạy đồng thời đọc/ghi cùng sheet dễ đua nhau đọc sai "dòng cuối" rồi ghi đè lên nhau, RỚT MẤT DỮ LIỆU (đã xảy ra thật 2 lần: seed 18 dòng Tiến độ chỉ còn 6-7 dòng; gửi 4 thông báo duyệt thiết bị chỉ còn 3, 2 dòng trùng cả ID). Luôn gộp thành 1 lệnh `*Batch` (server dùng `addDataBatch()` — 1 lệnh `setValues()` duy nhất) — xem `addNotificationsBatch` action + `TaskManager.addSystemNotificationsBatch()` làm mẫu.
- **Offline-first (chốt 2026-09-16, KIẾN TRÚC ĐÃ ĐỔI LỚN 2026-09-17 — đọc bản mới này, đừng theo mô tả cũ trong lịch sử)**: mọi trang PHẢI hoạt động xem-được khi mất mạng (iOS Safari + Android Chrome), và MỌI hàm ghi dữ liệu mới thêm sau này PHẢI tự chặn khi mất mạng — xem `public/js/offline.js` (`Offline.guard('nhãn hành động')` — gọi ở ĐẦU hàm, trả `true` = đã chặn + tự hiện toast, return sớm ngay). 4 lớp:
  1. **App shell offline** — `public/sw.js` (Service Worker) precache toàn bộ HTML/CSS/JS/icon, đăng ký qua `offline.js`. Đổi `CACHE_VERSION` trong `sw.js` mỗi khi thêm/đổi tên file tĩnh mới HOẶC muốn ép mọi client tải lại code mới ngay (thêm trang mới PHẢI thêm vào mảng `PRECACHE_URLS`, không thì trang mới không mở được lúc mất mạng). **Chiến lược cache 2026-09-17: NETWORK-FIRST cho MỌI request** (kể cả css/js, trước đó là "stale-while-revalidate" — luôn trả bản CŨ trước, đây chính là lý do code mới deploy không bao giờ hiện ngay trên điện thoại, phải đợi nhiều phút/thoát app mới thấy, người dùng phản ánh thật). Mọi fetch trong `sw.js` giờ có `{cache:'no-store'}` để bỏ qua cả HTTP cache trình duyệt, chỉ rơi về cache khi mạng thật sự lỗi. `offline.js` còn nghe sự kiện `controllerchange` — SW mới giành quyền kiểm soát thì tự `reload()` ĐÚNG 1 LẦN để trang nạp code mới ngay, không cần người dùng tự thoát app.
  2. **Banner + mốc giờ dữ liệu** — `offline.js` tự chèn banner cố định đầu trang khi mất mạng, hiện đúng giờ:phút:giây + ngày/tháng/năm của lần đồng bộ THÀNH CÔNG gần nhất (`localStorage['hiconique_last_sync']`, tự cập nhật ở `fetchFromAPI()` trong `task-data.js`). Xác định "mất mạng" qua ping định kỳ 15s vào Apps Script Web App — **2026-09-17: nâng ngưỡng từ 3 lên `CONSECUTIVE_FAILS_TO_GO_OFFLINE = 10` lần trượt LIÊN TIẾP** (~2.5 phút toàn trượt mới kết luận offline — 3 lần trước đó quá dễ báo động giả, 1 lần Apps Script chậm/cold-start dù mạng máy vẫn tốt 100% cũng đủ khoá hết thao tác ghi, người dùng phải tự reload trang mới hết, phản ánh rất khó chịu).
  3. **Có mạng lại KHÔNG còn reload cả trang** (2026-09-17, đổi từ hành vi cũ) — trước đó `window.location.reload()` khi vừa có mạng lại, xoá mất y nguyên form/dữ liệu đang gõ dở nếu đúng lúc mạng chập chờn 1 nhịp. Giờ chỉ gọi `TaskManager.silentRefresh()` (làm mới dữ liệu NGẦM vào localStorage, không đụng DOM/form đang mở) + 1 toast ngắn. `silentRefresh()` cũng tự chạy định kỳ 20s (`SILENT_REFRESH_MS`) và mỗi khi tab được focus lại — đây là cách DUY NHẤT trang tự cập nhật dữ liệu khi đang mở liên tục, KHÔNG reload định kỳ nữa (bản cũ có làm vậy, gây nháy màn hình liên tục cắt ngang thao tác, đã bỏ hẳn — xem thêm mục PWA/bfcache bên dưới, ĐÂY LÀ TRƯỜNG HỢP DUY NHẤT còn được reload cả trang).
  4. **Chặn ghi khi mất mạng** — `guard()` giờ chỉ chặn khi **CẢ HAI** tín hiệu cùng báo mất mạng (ping trượt đủ 10 lần **VÀ** `navigator.onLine === false`) — trước đó chỉ dựa 1 mình ping nên dễ chặn oan. Đã chặn sẵn ở các hàm dùng chung `add()`/`update()`/`remove()`/`callGSheetsAPI()` (`task-data.js`, hầu hết mọi loại dữ liệu đi qua đây) + các hàm ghi lẻ gọi thẳng API (`addAttendanceLocation`/`updateAttendanceLocation`/`deleteAttendanceLocation`/`addSystemNotificationsBatch`, `Auth.register()` trong `auth.js`, `gsWrite()` trong `hicon-bim.html`/`pricing.html`). Thêm 1 luồng ghi MỚI không đi qua các hàm này (VD gọi `fetch(GSHEETS_CONFIG.API_URL...)` thẳng ở 1 trang khác) → PHẢI tự thêm `if (typeof Offline !== 'undefined' && Offline.guard('...')) return;` ở đầu, không được bỏ qua.
  5. **Kéo xuống đầu trang = reload thủ công (2026-09-16)** — `initPullToRefresh()` trong `offline.js`, chỉ bắt gesture khi trang đang ở đỉnh (`scrollTop=0`) và kéo XUỐNG đủ ngưỡng, tự vẽ icon mũi tên xoay bằng SVG inline, không dùng thư viện ngoài.
- **Logo chính thức của công ty (chốt 2026-09-16)**: hình khối 3 thanh nghiêng kiểu skyline/kiến trúc (KHÔNG còn là chữ "H" đơn) — nguồn gốc `public/logo-soucre.png` (do người dùng cung cấp, giữ nguyên không xoá). Từ file gốc này sinh ra 2 biến thể dùng xuyên suốt web, KHÔNG tự vẽ lại/đổi màu khác khi thêm chỗ dùng logo mới:
  - `public/logo-mark.png` — bản màu đồng/vàng thương hiệu (`#B08D57`), nền trong suốt. Dùng ở mọi nơi có nền sáng/nhạt hoặc cần phối màu theo `--color-bronze`: logo header (`.brand-mark` + `.brand-logo` trong `portal.css`, và bản inline style riêng của `login.html`).
...
### Phiên 2026-09-24 — Fix "Tổng" tiến độ task (2 lần sửa, LẦN 2 mới đúng — đọc kỹ)

- **Triệu chứng gốc**: task 2 người (Khánh, Sáng) đã cập nhật 22/9=50%, 23/9=100% ("Xong") nhưng "Tổng" vẫn hiện 75%, nút "Hoàn thành — nộp duyệt" không sáng dù đã báo xong việc.
- **Sửa lần 1 (SAI, đã bị ghi đè)**: tưởng mỗi ngày nhập 1 MỐC TUYỆT ĐỐI (kiểu "hôm nay tiến độ đạt tới đâu"), nên đổi `task.progress` = lấy MAX của mọi ngày. Người dùng phản hồi lại: **sai tư duy** — mỗi ngày nhập là **% LÀM THÊM ĐƯỢC TRONG NGÀY ĐÓ** (số gia tăng riêng, không phải mốc tuyệt đối), "Tổng" phải là **CỘNG DỒN** các ngày (chặn trần 100%). VD ngày 1 làm 20%, ngày 2 làm thêm 30% → Tổng = 50%. Nếu 1 ngày báo hẳn 100% (coi như xong trong ngày đó) → Tổng = 100% (cộng dồn rồi chặn trần, không vượt quá) — khớp đúng ví dụ Khánh/Sáng ở trên (50+100=150, chặn trần còn 100).
- **Sửa lần 2 (ĐÚNG, đang áp dụng)**: `addDailyProgress()` (task-data.js) — mỗi lần lưu GHI ĐÈ đúng dòng hôm nay (sửa lại trong ngày thì thay số cũ, không cộng dồn thêm 1 lần nữa), `task.progress` = `sumDailyProgress_()` = tổng tất cả các ngày, chặn trần ở 100. Thêm `getTodayProgressCap()` = 100% − tổng % các ngày KHÁC, dùng làm TRẦN (max) cho thanh trượt "hôm nay" — không còn khoá SÀN theo mốc cao nhất kiểu cũ (sai) nữa. UI (`task-manager-app.js`/`projects.js`): nhãn slider đổi thành "Hôm nay làm thêm được bao nhiêu % (của cả việc)?", cảnh báo trần thay cho cảnh báo sàn.
- **Task cũ đã bị lệch "Tổng" (do bug gốc hoặc do sửa lần 1 sai) sẽ TỰ SỬA ĐÚNG ngay lần lưu tiến độ kế tiếp** (kể cả bấm lưu lại đúng giá trị đang có) — không cần migrate dữ liệu cũ hàng loạt.
...
- **Danh sách thành viên (Team, báo cáo chấm công, dropdown giao việc...) không còn theo thứ tự thô của Sheet** — từ khi đổi sang chèn bản ghi mới ở ĐẦU Sheet (mục "du lieu moi luon chen dau Sheet" phía dưới), thứ tự thô gần như ngẫu nhiên, không phản ánh đúng sơ đồ tổ chức (người dùng phản ánh "đầu mục chưa đúng thứ tự đã thống nhất" ở trang Chấm công). Thêm `sortMembersByLevel_()` trong `task-data.js`, áp ngay tại nguồn `getMembers()`/`getActiveMembers()` — sắp theo đúng `LEVELS.order` đã thống nhất (Founder → CEO → Giám đốc Bộ phận → Quản lý → Nhân viên), cùng cấp bậc thì theo tên. Sửa 1 chỗ, MỌI trang dùng 2 hàm này tự động đúng thứ tự, không cần sửa từng trang.
- **Modal "Báo cáo chấm công" (CEO, `timesheet.html`) không tự cập nhật khi có dữ liệu mới ghi lên Sheet trong lúc đang mở** (VD Apps Script tự động đóng ca sáng quên check-out, hoặc nhân viên chấm công ở máy khác) — trước đây chỉ tính 1 lần lúc bấm mở modal, `silentRefresh()` (chạy ngầm mỗi 5s, xem `offline.js`) tuy vẫn cập nhật `localStorage` đúng nhưng không ai gọi lại `renderAttendanceReportModal()`. Giờ lắng nghe sự kiện `hiconique:data-refreshed` — nếu modal đang mở (`#tsReportModal.active`) thì tự vẽ lại NGAY, giữ đúng màn đang xem (danh sách hay chi tiết 1 người, qua biến mới `reportCurrentMemberId`) thay vì bật ngược về danh sách.

### Phiên 2026-09-23 — Thông báo đẩy (Web Push) — popup thật trên điện thoại/máy tính giống Zalo

- **Kiến trúc (v2, BẢN CUỐI — không dùng service-account JSON)**: dùng Firebase Cloud Messaging (FCM) qua REST API `fcm.googleapis.com/v1/.../messages:send`. Lấy access token bằng `ScriptApp.getOAuthToken()` — chạy DƯỚI danh tính của chính Apps Script này (tài khoản sở hữu script = `hiconique.group@gmail.com`, TRÙNG tài khoản sở hữu project Firebase) — hoàn toàn KHÔNG cần tạo/tải/dán file service-account JSON nào. Chỉ cần khai `https://www.googleapis.com/auth/firebase.messaging` trong mảng `oauthScopes` của `appsscript.json` (mở file này qua Cài đặt dự án > tick "Hiển thị tệp kê khai appsscript.json...", checkbox này reset về false mỗi phiên trình duyệt mới nên phải tick lại nếu cần sửa manifest lần sau). **Bản v1 (JWT RS256 tự ký + service-account JSON dán vào Script Property `FCM_SERVICE_ACCOUNT_JSON`) đã bị BỎ HẲN** — nếu thấy code/note cũ nhắc tới `FCM_SERVICE_ACCOUNT_JSON`/`computeRsaSha256Signature`, đó là bản cũ, không dùng nữa.
- **Project Firebase**: tên "HICONIQUE Internal Hub", project ID **`hiconique-internal-hub-f77f2`**, tạo dưới tài khoản Google **hiconique.group@gmail.com** — xem [[reference_hiconique_links]]/`feedback_check_google_account_before_action` trong memory (lần đầu tạo nhầm dưới tài khoản cá nhân, đã xoá). Gói Spark (miễn phí), tắt Gemini/Analytics lúc tạo (không cần cho việc gửi push). Hằng số `FCM_PROJECT_ID` trong `gsheets-api-v2.js` phải khớp đúng Project ID này — đổi project Firebase sau này thì sửa hằng số đó + `firebaseConfig`/`VAPID_KEY` trong `public/js/push-notifications.js` và `public/sw.js`.
- **File đụng tới**:
  - `gsheets-api-v2.js`: sheet mới `TT-Thiết bị đăng ký thông báo` (`SHEETS.pushDevices`, cột `id/memberId/fcmToken/deviceLabel/browserFamily/active/createdAt/lastActiveAt`); action `registerPushDevice` (upsert theo `fcmToken`)/`unregisterPushDevice`; hàm `getFcmAccessToken_()` (chỉ 1 dòng `return ScriptApp.getOAuthToken()`)/`sendPushToMember_()`/`pushForNotificationRow_()` cuối file — móc DUY NHẤT vào `addData()`/`addDataBatch()` khi `sheetName === SHEETS.notifications`, nên MỌI đường tạo thông báo (client gọi `addNotification(sBatch)` HAY server tự ghi thẳng như `autoCheckoutForgottenEntries`/`notifyFounderMemberRejected`...) đều tự động kích hoạt gửi push, không phải sửa từng nơi.
  - `appsscript.json` (manifest Apps Script, không nằm trong repo git — chỉ tồn tại trong project Apps Script online): thêm `oauthScopes: [spreadsheets, script.external_request, firebase.messaging]`.
  - `public/js/push-notifications.js` (MỚI) — module `window.PushNotify` (`isSupported/getStatus/enable/disable/initAutoPrompt`), tự hỏi quyền 1 lần/trình duyệt bằng banner riêng (không dùng `confirm()`/`alert()` gốc), tự làm mới đăng ký token mỗi lần mở app nếu đã bật từ trước.
  - `public/js/portal.js` (`initNotifications()`) — gọi `PushNotify.initAutoPrompt(user)` ngay khi biết user, chạy ở MỌI trang có portal.js (17 trang).
  - `public/sw.js` — nạp Firebase Messaging qua `importScripts()` ở ĐẦU file (trước `CACHE_VERSION`), `onBackgroundMessage()` tự `showNotification()` khi tab đóng/nền, `notificationclick` tự focus tab có sẵn hoặc mở tab mới. `CACHE_VERSION` đã bump lên `v4`.
  - 17 trang `.html` (mọi trang có `portal.js`) — thêm 3 thẻ `<script defer>`: `firebase-app-compat.js`, `firebase-messaging-compat.js` (CDN gstatic, bản 10.12.2), rồi `/js/push-notifications.js` — đặt ngay TRƯỚC thẻ `portal.js`.
- **Chưa test round-trip thật với thiết bị thật** — việc tiếp theo khi có dịp: bấm "Bật thông báo" thử ở 1 trang, xác nhận token được ghi vào sheet `TT-Thiết bị đăng ký thông báo`, tạo 1 thông báo test (VD tự giao task cho mình) xem có popup thật hiện ra không, rồi dọn sạch dữ liệu test. Đã chạy thử `getProvinceList()` (hàm bất kỳ, không liên quan push) từ trình chỉnh sửa Apps Script sau khi thêm `oauthScopes` mới — chạy xong không bị chặn xin quyền lại, có vẻ ổn nhưng CHƯA chắc chắn 100% `ScriptApp.getOAuthToken()` thật sự trả về token có đủ quyền `firebase.messaging` cho tới khi test round-trip thật ở trên.

### Phiên 2026-09-22 (e) — Tiến độ hàng ngày chỉ tăng không giảm + nút "Nộp duyệt" sáng ở 95%

- **Thanh trượt "Cập nhật tiến độ hôm nay" giờ không kéo lùi lại được** (`openTaskDetail()` trong `public/js/projects.js`, `openTaskDetailModal()` trong `public/js/task-manager-app.js`) — trước đó `<input type="range" min="0">` cho phép kéo tự do về 0% dù hôm trước/hôm nay đã lưu cao hơn. Giờ tính `progressFloor` = % cao nhất từng lưu trong `dailyTasks` (mọi ngày, kể cả hôm nay nếu đã lưu 1 lần), gán làm `min` của input range — trình duyệt tự chặn cứng kéo xuống dưới mốc đó (không chỉ chặn ở JS). Có 1 thanh xám cứng (`.dps-slider-lock`, layout `display:flex` — thử `position:absolute` trước, bị lệch vì input range luôn tự căn thumb theo % TRONG box của chính nó chứ không theo % của cả hàng, phải đổi sang flex để thanh khoá + input chia đúng tỷ lệ width thật) hiện TRƯỚC vị trí đó để nhìn là biết "đã khoá tới đây, chỉ tăng thêm".
- **Nút "Hoàn thành — nộp duyệt" giờ LUÔN hiện (không thay bằng dòng chữ nhắc nữa)**, nhưng bị làm tối/khoá (`opacity:0.35; cursor:not-allowed; filter:grayscale(60%); disabled`) khi tiến độ < 95%, sáng lên + bấm được ngay khi đạt >= 95% (hạ từ ngưỡng cũ 100% — theo yêu cầu người dùng, không bắt buộc đúng 100% mới cho nộp). Đồng bộ ngưỡng ở CẢ 2 lớp: UI (2 file JS trên) VÀ server-side guard `submitTaskForReview()` trong `public/js/task-data.js` (trước đó chặn cứng `< 100` — nếu chỉ sửa UI mà quên chỗ này thì bấm nút ở 95-99% sẽ về null, im lặng không có tác dụng gì).

### Phiên 2026-09-22 (d) — 5 chế độ xem Ngày/Tuần/Tháng/Quý/Năm cho mục "Lịch" (cả tasks-manager.html lẫn projects.html)

- **Thêm bộ chuyển chế độ xem Ngày/Tuần/Tháng/Quý/Năm cho "Calendar To Do List"** ở CẢ 2 nơi có mục Lịch: `renderCalendar()` (`public/js/task-manager-app.js`, `todoCalState`) và `renderCalendarView()` (`public/js/projects.js`, `state.calViewMode`...) — theo đúng convention cố tình DUPLICATE logic giữa 2 file này đã có từ trước (không gộp thành module dùng chung). Mặc định vẫn là `'month'` nên hành vi cũ không đổi gì nếu không đụng vào nút chuyển chế độ.
  - **Ngày**: agenda 1 ngày — mũi tên lùi/tiến ngày + nhãn ngày dài tiếng Việt + nút "Hôm nay", khung chi tiết LUÔN hiện sẵn (không cần bấm chọn) kèm dòng thống kê "N việc cần làm hôm nay/ngày này".
  - **Tuần**: lưới 7 cột (T2→CN, đúng quy ước tuần bắt đầu Thứ 2 đã dùng ở lịch tháng) nhưng cột cao hơn hẳn (`.todo-cal-week-grid`, chỉ 1 hàng nên rộng chỗ) — hiện đủ pill task không giới hạn 3 như lưới tháng. Mũi tên lùi/tiến tuần + nhãn khoảng ngày.
  - **Tháng**: giữ NGUYÊN y hệt hành vi/markup cũ, chỉ bọc lại thành 1 nhánh chế độ (refactor thuần, không đổi output).
  - **Quý**: 3 mini-lịch tháng nằm cạnh nhau (`.todo-cal-quarter-grid` → 1 cột trên mobile) — mỗi mini chỉ hiện số ngày + 1 chấm bi nếu có task (không hiện tên task, quá chật với 3-up), hôm nay tô sáng, bấm 1 ngày mở đúng khung chi tiết dùng chung bên dưới. Mũi tên lùi/tiến quý (nhảy cả năm khi qua Quý 1/Quý 4) + dropdown Năm riêng.
  - **Năm**: y hệt kiểu mini-lịch nhưng đủ 12 tháng, lưới 4 cột responsive (`.todo-cal-year-grid`, xuống 1 cột ở mobile theo breakpoint `@media (max-width:640px)` có sẵn). Mũi tên lùi/tiến năm.
  - Khung chi tiết ngày (`.todo-cal-detail`/`.todo-cal-tasklist`, tick hoàn thành `TaskManager.toggleTaskStatus`, bấm tên mở modal chi tiết `openTaskDetailModal`/`openTaskDetail`) dùng CHUNG 1 hàm `buildDetailHtml()` cho mọi chế độ — hành vi giống hệt nhau bất kể ngày được chọn từ chế độ nào. `tasksDueOn()` (đã có sẵn) vẫn là nguồn xác định duy nhất "ngày này có task hay không", không viết lại logic đó ở đâu khác.
  - State mới: `todoCalState.viewMode/dayDate/weekAnchor` (task-manager-app.js) và `state.calViewMode/calDayDate/calWeekAnchor` (projects.js) — chỉ sống trong phiên (session), không lưu localStorage, y hệt cách `month`/`year`/`selectedDate` cũ đã hoạt động.
  - CSS mới (`.todo-cal-modeswitch`, `.todo-cal-mode-btn`, `.todo-cal-nav-btn`, `.todo-cal-nav-label`, `.todo-cal-week-grid`, `.todo-cal-day-stat`, `.todo-cal-quarter-grid`, `.todo-cal-year-grid`, `.todo-cal-mini-month`, `.todo-cal-mini-grid`, `.todo-cal-mini-day`, `.todo-cal-mini-dot`) thêm y hệt nhau vào CẢ `task-manager.css` lẫn `projects.css` (đúng chỗ khối `.todo-cal-*` cũ ở mỗi file) — cùng tên class, chủ đích duplicate giống toàn bộ khối Lịch đã có.
  - **Đã verify qua Claude in Chrome** (session đăng nhập thật, không phải mock) trên cả 2 trang `tasks-manager.html` và `projects.html`: bấm lần lượt cả 5 nút chế độ, bấm ô ngày ở Tuần/Quý mở đúng khung chi tiết, nút lùi/tiến Quý/Năm/Tuần/Ngày đổi đúng, không có lỗi console nào phát sinh từ code mới (chỉ còn lỗi cũ không liên quan `GSheets API read failed: AbortError` do máy dev không có mạng ổn định tới Apps Script — đã có từ trước, không phải do thay đổi lần này). Chế độ Tháng giữ nguyên y hệt giao diện cũ.

### Phiên 2026-09-22 (e) — Lưới giờ thật kiểu Google Lịch + zoom Ctrl/Cmd+lăn chuột qua các chế độ + vạch Sáng/Chiều/Tối cho mục "Lịch"

- **Chế độ Ngày/Tuần giờ có lưới giờ thật kiểu Google Lịch** thay cho danh sách/pill cũ — trục giờ 00:00→24:00 bên trái (48px/giờ), mỗi task đặt đúng vị trí theo GIỜ trong `deadline` (không chỉ ngày như trước); nếu task có `startDate` cùng ngày và trước `deadline` thì vẽ thành khối kéo dài từ giờ start tới giờ deadline (giống sự kiện Google Lịch có khoảng thời gian), chỉ có `deadline` thì vẽ khối "điểm" cao ~20-30 phút. **Việc chồng giờ nhau tự chia cột đứng cạnh nhau** (không đè lên nhau) — thuật toán: nhóm các khối chồng giờ thành từng cụm (cluster, dựa trên khoảng [startMin,endMin) giao nhau), trong mỗi cụm gán cột theo kiểu tham lam interval-graph-coloring (mỗi khối lấy cột trống sớm nhất còn rảnh tại giờ bắt đầu của nó), rồi chia đều bề rộng cột theo số cột đang dùng trong cụm đó — đúng kiểu Google Lịch vẫn làm. Tuần = 7 cột lưới giờ này đặt cạnh nhau dùng chung 1 trục giờ; Ngày = 1 cột rộng. Tháng/Quý/Năm giữ NGUYÊN kiểu tóm tắt chấm bi/pill cũ (đủ chi tiết thời gian thật không hợp với ô nhỏ).
  - Khung agenda/checklist chi tiết ngày (tick hoàn thành, bấm mở modal) VẪN giữ nguyên bên dưới lưới giờ — bấm 1 khối task trên lưới giờ mở đúng modal chi tiết, có ô tick nhỏ riêng trong khối để đánh dấu hoàn thành tại chỗ mà không cần mở modal.
- **Giữ Ctrl (hoặc Cmd trên Mac) + lăn chuột trên khung Lịch để zoom qua các chế độ xem** — dùng ĐÚNG modifier key + chiều cuộn đã có sẵn ở `HiconiqueGantt.bindWheelZoom()` (`public/js/gantt.js`, xác nhận lại bằng cách đọc thẳng code gantt: `e.ctrlKey || e.metaKey`, cuộn lên `deltaY<0` = zoom in) để thao tác đồng nhất toàn app thay vì tự bịa ra quy ước khác (người dùng không nhớ rõ Ctrl hay Shift nên yêu cầu kiểm tra lại). Bậc thang: **Ngày ← Tuần ← Tháng ← Quý ← Năm** (cuộn lên/zoom in = sang trái = chi tiết hơn, cuộn xuống/zoom out = sang phải); ở 2 đầu thang (đã ở Năm mà zoom out tiếp, hoặc đã ở Ngày mà zoom in tiếp) thì không làm gì, không lỗi không xoay vòng. `e.preventDefault()` khi có giữ Ctrl/Cmd để không cuộn cả trang, không giữ thì lăn chuột vẫn cuộn trang bình thường (không đụng gì hành vi cũ).
  - **Giữ đúng ngữ cảnh đang xem khi zoom** (không nhảy về "hôm nay") — `getCalRefDate()`/`setCalMode()` mới: suy ra 1 "ngày mốc" từ chế độ hiện tại (`dayDate`/`weekAnchor`/ngày đang chọn hoặc ngày 01 của tháng/quý/năm đang xem), rồi set state của chế độ MỚI dựa trên đúng ngày mốc đó — VD đang xem Tuần chứa 22/09/2026, zoom out 2 nấc sang Quý thì phải ra đúng Quý 3/2026 (chứa 22/09), không phải Quý 1. Đã test qua console xác nhận đúng hành vi này (xem mục Verify bên dưới).
- **2 vạch ngang phân Sáng/Chiều/Tối** trong lưới giờ Ngày/Tuần — lấy đúng mốc giờ làm việc mặc định của công ty đã có sẵn (`DEFAULT_WORK_SCHEDULE` trong `task-data.js`: `morningEnd: '11:30'`, `afternoonEnd: '17:30'`) thay vì tự bịa giờ, không đọc lại Sheet `TLCC-Giờ làm việc` tuỳ chỉnh (tính năng lịch không cần chính xác tuyệt đối theo giờ làm việc đã tuỳ chỉnh, chỉ cần đúng mốc mặc định để phân vùng trực quan) — vạch nét đứt màu đồng (bronze), tách biệt rõ với vạch giờ mảnh bình thường.
- CSS mới (`.todo-cal-timegrid-wrap/-headrow/-gutter/-headcells/-headcell/-scroll/-hours/-hourlabel/-days/-day`, `.todo-cal-band-divider`, `.todo-cal-event/-check/-title`) thêm y hệt nhau vào CẢ `task-manager.css` lẫn `projects.css`, dùng `var(--color-*)` sẵn có, không hardcode màu mới. Vạch giờ mảnh dùng `repeating-linear-gradient` trên nền thay vì vẽ 24 `<div>` riêng mỗi cột (đỡ tốn DOM khi Tuần = 7 cột × 24 vạch).
- Hàm mới trong CẢ 2 file (`task-manager-app.js`/`projects.js`, y hệt tên, duplicate đúng convention cũ): `taskTimeBlock()`, `layoutDayBlocks()`, `buildTimeGridDayCol()`, `buildTimeGrid()`, `getCalRefDate()`, `setCalMode()`, `bindCalZoomWheel()`.
- **Đã verify qua Claude in Chrome** (không dùng Browser pane sẵn có — theo đúng quy tắc cố định) trên cả `tasks-manager.html` và `projects.html`, chạy trên 1 server dev CỤC BỘ port riêng (3101, tách khỏi server port 3000 đang chạy sẵn trên máy để chắc chắn test đúng code của phiên này, không lẫn code cũ) vì phiên này chạy trong git worktree riêng: xác nhận Ngày/Tuần hiện đúng lưới giờ + 2 vạch Sáng/Chiều/Tối; tạo 3 task test chồng giờ nhau qua console (không ghi lên Sheet thật, chỉ patch tạm `TaskManager.getTasks` trong bộ nhớ trình duyệt rồi phục hồi lại sau khi xong) — xác nhận layout tự chia 3 cột đứng cạnh nhau đúng thuật toán; test Ctrl+lăn chuột qua console (`WheelEvent` giả lập `ctrlKey:true`) xác nhận đúng bậc thang zoom in/out cả 2 chiều, dừng đúng ở 2 đầu (Ngày và Năm), giữ đúng ngữ cảnh (Tuần chứa 22/09 zoom ra đúng Quý 3, không phải Quý 1); lăn chuột KHÔNG giữ Ctrl xác nhận không đổi chế độ (không phá hành vi cuộn trang bình thường); Tháng/Quý/Năm vẫn y hệt cũ (không regression); không phát sinh lỗi console mới từ code lịch (chỉ còn lỗi cũ không liên quan `GSheets API read failed: AbortError` do máy dev test không có mạng ổn định tới Apps Script).

### Phiên 2026-09-22 (c) — Tự động đóng ca sáng quên check-out lúc 12h30 + mặc định hiện chi tiết hôm nay trên lịch chấm công

- **Tự động đóng ca SÁNG quên check-out lúc 12h30 trưa mỗi ngày** (`gsheets-api-v2.js`) — trước đây CHỈ có 1 ngưỡng tự đóng ca ở CUỐI NGÀY (`autoCheckoutForgottenEntries()`, chạy 0h05 hôm sau, chỉ xử lý NGÀY ĐÃ QUA qua field `status`/`checkinTime`/`checkoutTime` cũ). Thêm `autoCheckoutForgottenMorningShifts()` MỚI — chạy 12h30 TRƯA CÙNG NGÀY, quét entry hôm nay có `morningCheckin` nhưng chưa có `morningCheckout` thì tự đóng NGAY (0 giờ công ca sáng), dùng CHUNG `AUTO_CHECKOUT_NOTE_TAG` với hàm cũ (client không cần sửa gì để đếm "số lần quên checkout"). Cài trigger riêng qua `setupAutoCheckoutMorningTrigger()` — **ĐÃ CHẠY 1 LẦN** từ Apps Script editor, log xác nhận "Đã cài trigger tự động đóng ca sáng — chạy hàng ngày lúc ~12h30." Apps Script đã redeploy **phiên bản 75**.
- **Lịch chấm công cá nhân mặc định hiện sẵn chi tiết NGÀY HÔM NAY** (`timesheet.html`) — trước đây `#tsDayDetail` ẩn (`display:none`) cho tới khi tự bấm vào 1 ô ngày. Giờ gọi `showDayDetail(getToday())` ngay sau `renderCalendar()` lúc tải trang VÀ khi bấm nút "Hôm nay" (quay về tháng hiện tại) — bấm ngày khác vẫn đổi đúng ngày đó bình thường (không đổi gì ở `showDayDetail()`).

### Phiên 2026-09-22 (b) — Fix bug đồng bộ 2 chiều + cột Ẩn/Hiện cho task-dự án + mở quyền tạo dự án + sửa cấp bậc thành viên

- **Fix bug "đổi trạng thái xong, rời trang quay lại bị trả về giá trị cũ"** (người dùng phản ánh: đổi task sang "Đang làm" xong, Sheet đã ghi nhận đúng, nhưng app hiện sai khi quay lại trang) — nguyên nhân: `updateTask()`/`updateProject()` ghi `localStorage` NGAY (đúng) nhưng chỉ BẮN request lên Apps Script qua `syncToGSheets()` (fire-and-forget, không chờ) — ghi thật trên Sheet có thể mất 1-3s+ (đặc biệt lúc Apps Script "cold start"). Nếu `refreshFromGSheets()` chạy TRONG lúc đó (chuyển trang = `initData()` tự gọi lại, hoặc `silentRefresh()` định kỳ 20s) thì bản GET trả về dữ liệu CŨ và bị `localStorage.setItem()` ghi đè lên bản local ĐÚNG vừa đổi. Sửa: `mergeServerData()` mới trong `task-data.js` — khi nhận dữ liệu từ Sheet, GỘP theo `updatedAt` từng bản ghi (giữ bản mới hơn, local hay server) thay vì ghi đè cả mảng, áp dụng cho `tasks`/`projects` (2 loại người dùng phản ánh, sửa nhiều nhất). Bản ghi CHỈ có ở local (mới tạo/mới ẩn, server chưa kịp phản ánh) cũng được giữ lại trong 5 phút (grace period).
- **Cột "Hiển thị" (visible) mới cho `tasks`/`projects`** (`FIELD_MAP` trong `gsheets-api-v2.js`) — nút "Xóa" giờ CHỈ ẨN (`visible=false`), KHÔNG xóa thật dòng Sheet nữa (theo yêu cầu người dùng — lỡ ẩn nhầm còn cứu được bằng cách vào thẳng Sheet sửa lại cột này). `deleteTask()`/`deleteProject()` trong `task-data.js` đổi hẳn sang soft-hide.
  - Task/dự án Hoàn thành tự ẩn khi qua khỏi THÁNG hoàn thành (trước đây là hết TUẦN, đổi theo yêu cầu người dùng) — `TaskManager.isVisibleNow(item)` (hàm lọc dùng ở mọi board/list UI: Kanban, "Tasks của tôi", nav badge, lịch...) + `autoHideExpiredCompleted()` (chạy 1 lần/ngày/máy sau mỗi `refreshFromGSheets()` thành công, quét task/dự án Hoàn thành đã qua tháng rồi ghi THẬT `visible=false` xuống Sheet qua 1 lệnh batch duy nhất — action mới `updateTasksBatch`/`updateProjectsBatch` gọi `updateDataBatch()` phía server, không loop gọi API đơn lẻ).
  - **Dữ liệu KHÔNG mất — thống kê năm/quý PHẢI đọc thẳng `TaskManager.getTasks()`/`getProjects()` (không lọc), KHÔNG dùng `isVisibleNow()`/wrapper đã lọc** (`visibleTasks()` trong `task-manager-app.js`, `getTasks()`/`getProjects()` wrapper trong `projects.js`) — 2 wrapper này CHỈ dùng cho board/list hiển thị hàng ngày.
  - Đã test round-trip đúng qua API thật: ghi/đọc `visible`, gọi `updateTasksBatch`, soft-delete qua `deleteTask()` (dữ liệu còn nguyên, `isVisibleNow()` đúng false), dọn sạch dữ liệu test khỏi Sheet bằng action `deleteTask`/`deleteProject` gốc (vẫn còn ở server, chỉ client không gọi nữa).
- **Mở quyền TẠO dự án cho mọi cấp bậc (kể cả Nhân viên)** — bỏ gate `roleLevel==='admin'||'manager'` ở `createProject()` (`task-data.js`), nút "Tạo dự án" trong `projects.js`/`task-manager-app.js`. Sửa/xóa dự án vẫn giữ nguyên chỉ CEO/quản lý (`canManageNotifications()`, không đụng tới — không được yêu cầu mở rộng).
- **Nút "Sửa cấp bậc" trong modal chi tiết thành viên** (`portal.js`, `showTeamMemberModal`/`openTeamMemberModal`) — CHỈ Founder (`level==='founder'`, đỉnh 5 mức) sửa được cấp bậc của **thành viên khác** (không tự sửa của chính mình qua nút này). `TaskManager.isFounder(user)` + `updateMemberLevel(id, levelCode, user)` mới trong `task-data.js` — set cả `level` và suy lại `roleLevel` tương ứng (`LEVELS` map). Click nút hiện dropdown chọn cấp bậc + Lưu/Hủy ngay tại chỗ (không dùng `prompt()`/`confirm()` gốc).
- Apps Script đã deploy phiên bản 74. **Lưu ý thao tác deploy phiên này**: dropdown "Phiên bản" trong hộp thoại "Quản lý các tùy chọn triển khai" click chuột hay bị chọn nhầm/không ăn (chọn "Phiên bản mới" nhưng lại quay về phiên bản cũ đang active) — cách ăn chắc: click mở dropdown rồi dùng phím mũi tên `Up`/`Down` + `Enter` để chọn thay vì click thẳng vào option.

- **Làm lại quy trình Task theo đúng 4 cột như Dự án, tiếng Việt hết** (`public/pages/tasks-manager.html`, `public/js/task-manager-app.js`, `public/js/task-data.js`, `gsheets-api-v2.js`) — quy trình mới: `Chờ xử lý` (quản lý/CEO giao việc cho người khác) → `Đang làm` (nhân viên xem + bấm "Xác nhận nhận việc"; nếu TỰ tạo task cho chính mình thì vào thẳng đây, bỏ qua Chờ xử lý) → `Chờ duyệt` (đủ 100% tiến độ, bấm "Hoàn thành — nộp duyệt") → CEO/Quản lý duyệt: **OK** → `Hoàn thành`; **Từ chối** → quay lại `Đang làm`, GIỮ NGUYÊN deadline, ghi lý do cần sửa vào cột `Ghi chú duyệt` (hiện thành khung cảnh báo đỏ trong modal chi tiết task cho nhân viên thấy).
  - Kanban đổi từ 3 cột (To Do/In Progress/Done) sang **4 cột tiếng Việt** giống hệt UI Dự án (`.kanban-column`/`.column-dot`/...): Chờ xử lý (chấm xám) / Đang làm (chấm xanh) / Chờ duyệt (chấm cam đất) / Hoàn thành (chấm xanh lá). Phát hiện ~20 task `status:'review'` có sẵn trong dữ liệu nhưng bị "vô hình" hoàn toàn ở board 3 cột cũ — không mất dữ liệu, chỉ thiếu cột hiển thị.
  - **Kéo-thả Kanban giờ chỉ CEO/Quản lý mới dùng được** (`TaskManager.canReviewTasks()` — `roleLevel==='admin'||'manager'`) — nhân viên thường không tự kéo thẻ đổi trạng thái được nữa (dễ bấm nhầm/bỏ qua quy trình xác nhận-nộp duyệt), phải dùng đúng các nút trong modal chi tiết task. CEO/Quản lý vẫn kéo-thả tự do như 1 cách override thủ công.
  - Hàm mới trong `task-data.js`: `confirmTaskAssignment()`, `submitTaskForReview()` (chặn nếu `progress<100`), `approveTaskReview()`, `rejectTaskReview(taskId, note, user)`, `canReviewTasks()`. `createTask()` tự quyết trạng thái ban đầu (không còn nhận `status` từ form lúc tạo mới) + tự gửi thông báo "Bạn được giao việc mới" cho người được giao (trừ người tự giao cho mình).
  - Thêm cột `['Ghi chú duyệt', 'reviewNote']` vào `FIELD_MAP.tasks` trong `gsheets-api-v2.js` — **ĐÃ REDEPLOY phiên bản 73**, xác nhận round-trip đúng qua API thật (ghi/đọc lại/xoá sạch dữ liệu test trên Sheet, không để sót).
  - **Bẫy clipboard phát hiện khi paste file vào Apps Script editor lần này**: `Set-Clipboard` (PowerShell) rồi mới chuyển sang trình duyệt để `Ctrl+V` KHÔNG an toàn tuyệt đối — clipboard hệ điều hành có thể bị ghi đè bởi hoạt động khác (kể cả người dùng tự copy thứ gì đó trên máy) đúng lúc giữa 2 bước, từng dính 1 lần paste nhầm hẳn nội dung không liên quan. Cách an toàn: `Set-Clipboard` + `Get-Clipboard -Raw` xác nhận lại NGAY trong cùng 1 lệnh PowerShell, rồi bấm Ctrl+V ngay lập tức không xen bước nào khác ở giữa; luôn Ctrl+F tìm 1 chuỗi ASCII đặc trưng vừa thêm để xác nhận paste đúng trước khi Lưu/Triển khai.
  - Đã test toàn bộ state machine (xác nhận nhận việc/nộp duyệt/duyệt/từ chối, phân quyền đúng-sai người) qua cả UI thật lẫn gọi thẳng hàm `TaskManager.*`, dọn sạch mọi task test khỏi Sheet thật sau khi xong.
...
- **"Vân tay trình duyệt" (device fingerprinting) kết hợp với deviceId ngẫu nhiên** (`timesheet.html` `computeDeviceFingerprint()` + `task-data.js` `registerMemberDevice()`) — deviceId ngẫu nhiên (UUID lưu localStorage) vẫn là định danh CHÍNH; vân tay (băm thô từ trình duyệt/OS/màn hình/múi giờ, KHÔNG đọc phần cứng thật — web không có quyền đó) chỉ là lớp NHẬN LẠI PHỤ: khi 1 deviceId lạ xuất hiện nhưng vân tay trùng 1 thiết bị CỦA CHÍNH người đó đã duyệt trước (VD nhân viên xoá dữ liệu trình duyệt làm mất deviceId cũ), hệ thống tự THAY id, giữ nguyên trạng thái đã duyệt — không bắt chờ duyệt lại, có báo cho CEO/Quản lý biết để tự kiểm tra nếu nghi ngờ. Đồng thời "Quản lý tất cả thiết bị" hiện cảnh báo ⚠️ nếu 2 NGƯỜI KHÁC NHAU có vân tay trùng nhau (dấu hiệu nghi chấm công hộ) — chỉ là gợi ý, không tự chặn. **Cố tình KHÔNG tự vá vân tay cho thiết bị cũ khi mở trang** (vi phạm quy tắc "không tự ghi dữ liệu chung như tác dụng phụ của việc mở trang" đã chốt trước đó) — vân tay chỉ được ghi vào đúng lúc có hành động ĐĂNG KÝ rõ ràng (bấm nút, hoặc admin thêm thủ công), nên thiết bị đăng ký từ trước phiên này sẽ tự có vân tay ở lần đăng ký lại kế tiếp (nếu có), không hồi tố ngay.
...
- **Nút "Setup thời gian làm việc"** (đúng ô trống cạnh "Xuất báo cáo"/"Xem báo cáo"/"Quản lý địa điểm GPS/IP" trong `timesheet.html`) — cấu hình giờ chuẩn ca sáng/chiều (mặc định 07:30–11:30 / 13:30–17:30), làm mốc tính đi muộn/về sớm, sửa được bất cứ lúc nào ngay trong web. Sống trên sheet riêng `TLCC-Giờ làm việc` (luôn đúng 1 dòng, upsert phía server qua action `getWorkSchedule`/`saveWorkSchedule`).
  - **Giờ nhập LUÔN 24h** — cố tình KHÔNG dùng `<input type="time">` (hiện 12h/24h tuỳ định dạng hệ điều hành/trình duyệt người dùng, người dùng phản ánh thấy "07:30 SA"/"01:30 CH" khó đọc) mà dùng `<input type="text">` + tự mask số khi gõ (`bindTimeTextMask()`, gõ "0730" tự hiện "07:30") + validate regex `HH:MM` trước khi lưu.
  - **Chỉ Founder/CEO (`user.level`) mới sửa/lưu được, các cấp còn lại (kể cả Giám đốc Bộ phận dù cùng `roleLevel:'admin'`) bấm vào chỉ XEM** (input `readOnly`, ẩn nút Lưu, hiện dòng "Chỉ Founder/CEO mới chỉnh sửa được — bạn đang xem").
  - **Bẫy phát hiện khi làm phần phân quyền này**: `Auth.saveSession()` (`auth.js`) từ trước tới giờ chỉ copy `roleLevel` (3 mức cũ) vào session lưu `localStorage`, KHÔNG copy `level` (5 mức mới, thêm 2026-09-16) — mọi chỗ check `user.level` ở CLIENT (kể cả tính năng này) luôn ra `undefined` với session đăng nhập TỪ TRƯỚC khi có field `level`, chỉ đúng nếu đăng xuất/đăng nhập lại. Đã thêm `level` vào `saveSession()` + vá thêm session đang cache ngay trong `reconcileStaleSession()` (so `session.level` với bản ghi Thành viên hiện tại, ghi đè nếu khác) để không phải đợi hết hạn 24h/đăng xuất mới có hiệu lực.
- **Tách Check In/Check Out thành 2 khối riêng theo ca** (Ca sáng/Ca chiều) thay cho 1 nút chấm công/ngày duy nhất trước đây — vẫn giữ **1 dòng/ngày** trong sheet `TLCC-Chấm công` (không tách 2 dòng/ca), chỉ thêm 4 cột giờ vào/ra từng ca + `isLate`/`isEarly`/`lateEarlyNote` vào `FIELD_MAP.timesheet`. Mirror `morningCheckin`→`checkinTime` và `afternoonCheckout`→`checkoutTime` (field cũ) để lịch/báo cáo/thống kê tháng đã có không cần sửa gì thêm. Ca chiều khoá cho tới khi ca sáng check-out xong.
- **Đi muộn/về sớm: bắt buộc nhập lý do TRƯỚC KHI cho chấm công tiếp** (so giờ hiện tại với giờ chuẩn ở client, `isLateOrEarly()`) — ô Ghi chú tự đổi nhãn "Lý do đi muộn/về sớm (bắt buộc)" + viền đỏ + chặn submit tới khi có nội dung (`requireLateEarlyNote()`). Sau khi qua được bước lý do mới chạy tiếp `verifyThenProceed()` (GPS/Wifi/Thiết bị, dùng lại y nguyên, không đổi gì). Đồng thời **gửi thông báo cho Founder/CEO/Quản lý** (`shiftCheckIn`/`shiftCheckOut` trong `task-data.js`, gọi `addSystemNotificationsBatch(adminAndManagerMembers(...))` — 1 lệnh batch, không loop) và **hiện rõ trạng thái trên lịch/báo cáo**: badge "Đi muộn"/"Về sớm" (kèm lý do) ở modal chi tiết ngày (`showDayDetail()`), cột "Muộn/Sớm" mới trong Excel xuất báo cáo (`exportAttendanceReport()`, tô đỏ khi có).
- **Bug NGHIÊM TRỌNG phát hiện khi test tính năng này — ÂM THẦM MẤT TRẮNG DỮ LIỆU, không riêng tính năng chấm công**: `addData()`/`updateData()` (`gsheets-api-v2.js`) chỉ ghi field có header ĐÃ CÓ SẴN trên Sheet (`headers.forEach`/`headers.map`) — nhánh tự tạo header từ `FIELD_MAP` chỉ chạy khi sheet HOÀN TOÀN RỖNG (`headers.length === 0`). Sheet `TLCC-Chấm công` đã có dữ liệu từ trước nên 9 cột MỚI vừa thêm vào `FIELD_MAP.timesheet` (giờ ca sáng/chiều, đi muộn, về sớm, lý do...) bị ÂM THẦM BỎ QUA khi ghi — không lỗi, không cảnh báo, chỉ là local cache (localStorage) có dữ liệu còn Sheet thật thì KHÔNG, phát hiện được nhờ tự kiểm tra lại Sheet sau khi test thay vì chỉ tin kết quả trả về từ client. **Đây là bẫy chung cho MỌI lần thêm field mới vào FIELD_MAP của 1 sheet ĐÃ CÓ SẴN dữ liệu** — không riêng chấm công. Đã thêm `ensureSchemaColumns(sheet, sheetName, headers, dataObj)`: tự bổ sung cột còn thiếu trên Sheet (CHỈ field đã khai báo trong `FIELD_MAP` và có mặt trong dữ liệu đang ghi — không tự tạo cột cho field lạ/gõ nhầm) ngay trong `addData()`/`updateData()`, không cần nhớ chạy tay 1 lần migrate mỗi khi thêm field mới nữa.
- Apps Script đã deploy phiên bản 69 (69 = phiên bản 68 thêm schema `TLCC-Giờ làm việc` + field mới, cộng thêm fix `ensureSchemaColumns` ở trên). Đã test trực tiếp trên trình duyệt (dev server local): cấu hình giờ chuẩn lưu/đọc lại đúng, chấm công cố tình muộn bị chặn đúng cho tới khi nhập lý do, dữ liệu ghi đúng đủ cột trên Sheet thật (xác nhận bằng cách mở Sheet xem trực tiếp sau khi test), thông báo tới đúng 2 Quản lý nhận được đúng nội dung. Đã dọn sạch toàn bộ dữ liệu test (dòng chấm công + thông báo test) khỏi Sheet thật.
- **Lưu ý môi trường test**: sandbox trình duyệt dùng để test phiên này có kết nối chập chờn thật sự tới `script.google.com` (request đôi khi trả 503/404/treo lâu rồi mới thành công) — không phải bug code, đã xác nhận lại bằng cách gọi trực tiếp cùng 1 request nhiều lần đều thành công sau khi thử lại.

### Phiên 2026-09-18 — Màu sắc đại diện tự sửa + chuyển "Nhắc định kỳ" sang trang Thông báo + fix bug ghi `false`

- **"Màu sắc đại diện" (cột Sheet NS-Thành viên, field `color`) giờ đồng bộ đúng vào MỌI avatar của chính người dùng** — trước đó avatar header (`.avatar`)/dropdown tài khoản (`.user-menu-avatar`) luôn dùng màu CSS cố định (navy/bronze), bỏ qua hẳn `color` dù avatar Team/Task/Project (`m.color`) đã dùng đúng từ lâu. Sửa ở đúng 1 chỗ dùng chung `initUserMenu()` (`public/js/portal.js`) nên tự áp dụng cho MỌI trang cùng lúc, không cần sửa từng trang — đọc `TaskManager.getMembers()` tươi thay vì `session.color` cũ để không bị lệch khi vừa đổi màu xong.
- **Trang Thông tin cá nhân (`profile.html`) có ô tự chỉnh "Màu sắc đại diện"** (đúng ô trống cạnh "Quê quán" mà người dùng khoanh đỏ) — color-picker + 9 màu preset (dùng chung bảng màu với màu dự án ở `projects.js`), ai cũng tự sửa được màu của chính mình (thêm `color` vào `MEMBER_SELF_EDIT_FIELDS` trong `task-data.js`). Sau khi lưu, vá luôn session đang cache qua `Auth.updateSessionFields()` (hàm mới) + set trực tiếp style avatar header/profile — đổi màu thấy ngay trên trang đang mở, không cần F5/đăng nhập lại.
- **Fix luôn 1 vi phạm "avatar phải vuông bo góc" có sẵn từ trước**: `.user-menu-avatar` (avatar lớn trong dropdown tài khoản) đang là hình TRÒN (`border-radius: 50%`, sót khỏi danh sách override) — đã thêm vào khối CSS `!important` chung trong `portal.css`.
- **Bỏ "Nhắc định kỳ hiện có" khỏi panel chuông thông báo, chuyển hẳn sang trang Thông báo** (`notices.html`, mục "Quản lý nhắc định kỳ" — CHỈ Founder/CEO/GĐ Bộ phận thấy, `TaskManager.canManageRecurringRules()`) — panel chuông giờ chỉ còn thông báo cá nhân, gọn hơn. Trang Thông báo giờ có: danh sách nhắc định kỳ (bật/tắt, Sửa, Xoá) + form thêm mới (tiêu đề/nội dung/khoảng ngày trong tháng) + form Sửa (bấm "Sửa" tự điền lại form, đổi nút thành "Lưu thay đổi" + "Hủy sửa"). Dùng lại đúng `showConfirmDialog()`/`showToast()` tự dựng (không phải `confirm()` gốc) — tiện thể thay luôn 1 chỗ sót `confirm()` gốc có sẵn từ trước ở nút Xoá thông báo (notice board) trên cùng trang.
- **Bẫy nghiêm trọng phát hiện khi test tính năng Bật/Tắt nhắc định kỳ**: `updateData()` trong `gsheets-api-v2.js` ghi field bằng `cell.setValue(forceTextIfDateLike(val, enKey) || '')` — khi `val` là boolean `false` (VD tắt 1 nhắc định kỳ), `false || ''` cho ra chuỗi RỖNG chứ không phải `false`, đọc lại KHÔNG BAO GIỜ ra đúng `false` nữa (mọi so sánh `=== false` ở client hỏng theo, âm thầm — không báo lỗi gì). Đây là bug ẩn ảnh hưởng MỌI field boolean ghi qua `updateData()` trong toàn hệ thống (không riêng nhắc định kỳ), chỉ chưa ai để ý vì hiếm khi có field boolean cần set về `false` qua đường update. Đã sửa: chỉ coalesce về `''` khi giá trị thật sự `undefined`/`null`, còn lại giữ nguyên (kể cả `false`). Đã deploy Apps Script phiên bản 66 + xác nhận qua API thật: tắt 1 nhắc định kỳ → đọc lại đúng `active: false` kiểu `boolean`, không còn ra `""`.
- **Nút "tự chọn màu" (input[type=color] gốc) giờ có icon bút nổi bật** ở cả "Màu sắc đại diện" (profile.html) và "Màu nhận diện" dự án (projects.html) — trước đó nút này trông y hệt các ô màu preset bên cạnh, dễ bấm nhầm. Icon bút trắng + đổ bóng đen (luôn nổi rõ bất kể màu nền), CSS dùng chung `.color-input-wrap`/`.color-input-pencil` trong `portal.css` (nạp ở mọi trang) — thêm 1 chỗ dùng color-picker mới sau này chỉ cần bọc `<input type="color">` trong `<span class="color-input-wrap">` + thêm SVG bút, không cần viết lại CSS.
- **Thêm "Tên viết tắt đại diện" (field `avatar`, 2 chữ hiện trên mọi avatar) vào trang Thông tin cá nhân, tự sửa được** — đặt ngay cạnh Quê quán (đẩy "Màu sắc đại diện" xuống dòng dưới). Rỗng thì tự suy lại từ tên (2 chữ cái đầu họ+tên cuối, giống cách avatar được sinh mặc định khi đăng ký). Đã thêm `avatar` vào `MEMBER_SELF_EDIT_FIELDS` (`task-data.js`) — cột Sheet đã có sẵn ("Tên viết tắt đại diện" → `avatar`), không cần đổi Apps Script.
- **UI dòng "Vị trí GPS" trong bảng xác thực chấm công** (`timesheet.html`) — nút "Thử lại"/"Cấp quyền vị trí" trước đó trôi nổi cạnh cả khối 3 dòng địa chỉ (chữ dài xuống dòng làm nút lệch giữa trông rất lạc quẻ). Đã gộp nhãn "Vị trí GPS" + nút vào chung 1 dòng đầu (`.ts-verify-head`); dòng địa chỉ/thông báo xuống dòng riêng bên dưới full-width. Thử căn giữa riêng nhãn "Vị trí GPS" trong hàng của nó trước — người dùng phản hồi lại muốn nhãn thẳng hàng CANH TRÁI với "Mạng Wifi công ty"/"Thiết bị đã đăng ký" (3 nhãn cùng 1 cột) thay vì tự căn giữa lệch nhau, nên đã đổi lại. Nút chữ "Thử lại"/"Cấp quyền vị trí" cũng đổi thành 1 icon reload nhỏ (SVG, tự xoay khi đang gọi lại định vị) nằm ngay cạnh nhãn thay vì nút chữ to — nhãn tooltip/aria-label vẫn phân biệt đúng "Cấp quyền vị trí" (khi bị từ chối quyền) và "Thử lại định vị" (timeout/không xác định được). Đổi luôn `.ts-verify-row` từ `align-items: flex-start` sang `center` — trên màn hẹp, thông báo dài như "Trang web chưa được cấp quyền vị trí" xuống 2 dòng khiến chấm tròn trạng thái dính cứng ở mép trên trông lệch hẳn lên; giờ chấm tròn luôn nằm đúng giữa chiều cao cả khối bất kể nhãn/thông báo dài ngắn.
  - **Bẫy phát hiện khi làm phần canh giữa này**: `.ts-checkin-card` có sẵn `text-align:center` cho khối đồng hồ — `.ts-verify-label`/`.ts-verify-state` (flex item con của `.ts-verify-body`, tự dãn full-width) vốn dĩ ĐÃ canh giữa đúng nhờ kế thừa CSS đó, KHÔNG cần tự thêm `text-align:center`. Nhét icon "Thử lại" chung vào `.ts-verify-body` làm body hàng GPS hẹp hơn 2 hàng kia (do icon chiếm chỗ) → tâm canh giữa bị lệch dù nhìn tưởng đúng — đã thử 2 cách sai trước khi ra bản đúng: (1) tự thêm `text-align:center` cho riêng nhãn GPS trong khi 2 hàng kia không có gây lệch tâm theo kiểu khác, (2) đẩy icon ra làm sibling cuối hàng + thêm ô trống vô hình cùng kích thước ở 2 hàng kia để giữ `.ts-verify-body` bằng nhau — ĐÚNG nhưng icon nằm cách xa nhãn (người dùng muốn icon SÁT cạnh nhãn). Bản CUỐI: bọc nhãn+icon trong `.ts-verify-label-row` (`display:flex; justify-content:center`) làm CẢ CẶP tự canh giữa như 1 khối trong `.ts-verify-body` full-width — icon ẩn thì khối tự co lại đúng vị trí canh giữa của riêng nhãn (khớp 2 hàng không icon), icon hiện thì áp sát cạnh nhãn mà cả cặp vẫn canh giữa. Đã đo bằng `getBoundingClientRect()` xác nhận `.ts-verify-body` cả 3 hàng rộng tuyệt đối bằng nhau.
- **Bug NGHIÊM TRỌNG tìm ra ngay sau khi push phiên trước (người dùng phản ánh "cả điện thoại và máy tính không đăng ký được thiết bị nữa")**: `task-data.js` có **2 hàm CÙNG TÊN `refreshFromGSheets`** (1 bản thêm 2026-09-10 có nhận `callback`, 1 bản thêm 2026-09-17 cho `silentRefresh()` KHÔNG nhận callback) — JS cho bản khai báo SAU thắng, nên `TaskManager.refreshFromGSheets` (export dùng ở nút "Đăng ký thiết bị này") thực chất luôn là bản KHÔNG hỗ trợ callback, khiến `TaskManager.refreshFromGSheets(doRegister)` không bao giờ gọi lại `doRegister` — nút kẹt vĩnh viễn ở "Đang đăng ký..." trên MỌI thiết bị (không phải do mạng hay do 1 máy cụ thể, bug này tồn tại từ 2026-09-17, không phải do các thay đổi phiên này). Đã gộp lại thành 1 hàm duy nhất (đủ 16 loại dữ liệu của cả 2 bản + hỗ trợ callback).
- **Bug thứ 2 phát hiện cùng lúc, gây "Vị trí GPS: Đang tắt" giả**: `gsCache.lastFetch` là 1 mốc giờ DÙNG CHUNG cho MỌI loại dữ liệu (projects/tasks/members/attendanceLocations/...) thay vì riêng từng loại — hễ bất kỳ loại nào khác fetch thành công thì mốc giờ chung nhảy lên mới, khiến `attendanceLocations` (không nằm trong danh sách `refreshFromGSheets()` làm mới định kỳ) bị "khoá cứng" ở kết quả CŨ — kể cả kết quả RỖNG do 1 lần lỗi/timeout — suốt cả phiên mở trang. `checkGeoStatus()`/`checkIpStatus()` (`timesheet.html`) đọc trúng cache rỗng giả này, tưởng nhầm "Chưa cấu hình địa điểm/IP" rồi hiện chữ "Đang tắt" (dùng chung 1 chữ cho MỌI lý do skip) dù dữ liệu thật vẫn còn nguyên trên Sheet. Đã đổi sang mốc giờ RIÊNG từng loại (`gsCacheTime[type]`), và không cho mảng RỖNG (lỗi/timeout) ghi đè lên cache TỐT đã có trước đó — áp dụng cả cho `attendanceLocationsCache` riêng của `timesheet.html`. Đã test lại trực tiếp: "Đang tắt" biến mất, nút "Đăng ký thiết bị này" đăng ký thành công ngay.
- **Rà lại toàn bộ phần Chấm công theo phản ánh "đăng ký thiết bị mãi không được" + "kiểm tra chấm công lâu hơn 1-2 phút"** (`timesheet.html`, `task-data.js`) — tìm ra 3 bug thật, không phải 1:
  1. **`registerMemberDevice()` LUÔN báo `ok:true` bất kể `updateMember()` có ghi được hay không** — nếu bị `Offline.guard()` chặn oan (mạng di động chập chờn, đã ghi ở quy tắc offline-first phía trên) thì KHÔNG có gì được ghi thật nhưng UI vẫn hiện "Đã đăng ký thiết bị thành công", người dùng tưởng xong, tải lại trang thấy mất — đúng cảm giác "đăng ký mãi không được". Đã sửa: check đúng kết quả `updateMember()` trước khi báo thành công, thêm nhánh báo lỗi rõ ràng ("Không đăng ký được — mạng đang chập chờn") thay vì im lặng. Áp dụng tương tự cho nút "Gỡ" thiết bị.
  2. **`fetchFromAPI()` (đường đọc dữ liệu Sheet DUY NHẤT cho TOÀN BỘ app, không riêng Chấm công) không có timeout** — mạng di động chập chờn có thể treo 1 request rất lâu (hàng chục giây tới cả phút) trước khi trình duyệt tự bỏ cuộc, đây mới là nguyên nhân chính gây "kiểm tra lâu hơn 1-2 phút" (không chỉ do GPS). Đã ép timeout cứng 8s bằng `AbortController`. `checkIpStatus()` (`timesheet.html`) cũng thiếu timeout tương tự cho fetch tới `api.ipify.org` — đã thêm timeout 6s.
  3. Giảm bớt độ trễ tự thêm ở fix GPS phiên trước: `getBestGeoPosition()` từ 3 lần thử/7s xuống còn 2 lần/5s (nguyên nhân gốc gây sai lệch km đã fix ở toạ độ Sheet, lớp retry giờ chỉ còn phụ, không cần thử nhiều/lâu).
- **"Cùng 1 điện thoại nhưng Safari và app 'Thêm vào Màn hình chính' bị tính là 2 thiết bị khác nhau"** — ĐÂY LÀ GIỚI HẠN THẬT CỦA NỀN TẢNG (iOS/Android cố tình cô lập storage giữa trình duyệt thường và PWA standalone vì lý do riêng tư, không có cách nào hợp nhất từ phía web). Đã thêm tính năng **"Đồng bộ mã thiết bị"** trong bảng thiết bị (`renderDeviceTable()`) — nút ẩn/hiện "Cùng 1 máy nhưng bị tính thành 2 thiết bị?" mở ra ô hiện mã thiết bị hiện tại (nút Sao chép) + ô dán mã từ "nơi kia", dán vào rồi bấm Đồng bộ sẽ ghi đè `localStorage['hiconique_device_id']` bằng mã vừa dán — 2 nơi từ đó dùng chung 1 danh tính, không tốn thêm slot trong tối đa 2 thiết bị. Nhắc người dùng gỡ đăng ký trùng cũ (nếu có) trước khi đồng bộ.
- **Fix bug thật gây "GPS thỉnh thoảng nhảy" / "cách văn phòng 9942450m"** (`timesheet.html`) — người dùng phản ánh cả điện thoại có GPS tốt cũng bị, chứng tỏ không phải do thiết bị. Nguyên nhân thật: cột `lat`/`lng` trong sheet "Địa điểm chấm công" gặp ĐÚNG bug locale y hệt IP đã ghi ở trên — client gửi lat/lng dạng CHUỖI (`<input>.value` luôn là string), Apps Script ghi thẳng vào ô không ép định dạng Text trước, Sheet locale VN đọc "." là dấu phân cách hàng nghìn nên "20.926738" bị lưu thành SỐ `20926738` (mất hết phần thập phân) — toạ độ ĐÍCH lệch hàng chục triệu lần khiến tính khoảng cách GPS ra sai lệch gần 10.000km, xảy ra với MỌI thiết bị (lỗi nằm ở toạ độ văn phòng lưu trên Sheet, không phải GPS máy người chấm công). Đã thêm `lat`/`lng` vào `FORCE_TEXT_FIELDS` (`gsheets-api-v2.js`), deploy Apps Script phiên bản 67, và sửa lại tay 2 dòng toạ độ văn phòng đã bị hỏng sẵn trên Sheet thật. Xác nhận qua test: từ "9942450m" sai bét → "15666m" (con số hợp lý, do định vị mạng của thiết bị test chỉ chính xác cỡ chục km, không phải bug nữa).
- **Thêm lớp bảo vệ phụ + nút "Cấp quyền vị trí"/"Thử lại" cạnh dòng Vị trí GPS** (không phải nguyên nhân chính ở trên, nhưng vẫn hữu ích): `checkGeoStatus()` giờ tự gọi lại `getCurrentPosition()` tối đa 3 lần (`getBestGeoPosition()`) giữ lại kết quả có `accuracy` tốt nhất; thêm ngưỡng "vô lý" `GEO_MAX_PLAUSIBLE_DISTANCE_M = 20000` — nếu khoảng cách tính ra vượt ngưỡng này (chắc chắn là lỗi định vị, không có công trình nào xa vậy) thì không hiện số mét hoang mang mà hiện *"Định vị không chính xác, bấm 'Thử lại'"*. Nút cạnh dòng Vị trí GPS tự đổi nhãn: **"Cấp quyền vị trí"** khi `err.code===1` (bấm nút gọi lại `getCurrentPosition()` — đây chính là cách web "xin lại quyền" với nền tảng/thiết bị, trình duyệt tự hiện lại hộp thoại nếu chưa bị chặn cứng), còn lại là **"Thử lại"** (timeout/không xác định được). Tính năng "tự chọn địa điểm GPS gần nhất khi có nhiều công trường" người dùng hỏi thêm — ĐÃ CÓ SẴN từ trước (`checkGeoStatus()` so khoảng cách tới TỪNG địa điểm đang bật, lấy điểm gần nhất), không cần code thêm.
- **Lưu ý khi test (không phải bug code)**: trong lúc AI đang test tính năng Nhắc định kỳ bằng cách tự tạo/xoá dòng qua console trên cùng Sheet thật, người dùng CŨNG đang tự tay thêm dòng test ("sà", "gfdgfdgdfg") song song → 2 bên cùng ghi vào 1 sheet sống cùng lúc khiến danh sách hiện dòng lạ xen kẽ, gây cảm giác "thêm bị ghi đè"/"xoá mất hết". Đã xác minh lại: nút Xoá chỉ xoá đúng 1 dòng được chọn, không ảnh hưởng dòng khác. Rút kinh nghiệm: KHÔNG tự test tạo/xoá dữ liệu qua console trên Sheet thật khi biết người dùng đang thao tác cùng lúc trên cùng tính năng.
- **Làm lại toàn bộ UI "Quản lý nhắc định kỳ" (`notices.html`) — bản trước bị chê "xấu"** (2 khối rời rạc lệch tỉ lệ, form không nhãn, "Từ ngày"/"Đến ngày" chữ bị cắt, nút "+ Thêm nhắc định kỳ" hiện như CHỮ THƯỜNG không viền/không nền). Gộp lại thành 1 khối `.recur-card` bo góc, CSS grid 2 cột (danh sách trái/form phải, có đường kẻ dọc phân cách), form giờ có nhãn (TIÊU ĐỀ/NỘI DUNG/TỪ NGÀY/ĐẾN NGÀY) phía trên từng ô thay vì chỉ dựa placeholder, mỗi dòng danh sách có chấm tròn xanh/xám báo Bật/Tắt. **Phát hiện thêm bug**: trang `notices.html` không nạp `task-manager.css` (nơi định nghĩa gốc `.btn-primary`/`.btn-secondary`) nên MỌI nút dùng 2 class này trên trang (kể cả nút "Đăng"/"+ Đăng thông báo mới" có từ trước, không riêng nút mới thêm) đều hiện ra như chữ thường — đã định nghĩa lại 2 class này ngay trong `<style>` riêng của trang. Đã test cả desktop lẫn mobile (375×812), 2 cột tự rớt xuống 1 cột đúng khi màn hẹp.
- **Sửa UI "Quản lý nhắc định kỳ" (`notices.html`) bị chữ đè lên nhau** — `.notice-date` (dùng chung cho ngày đăng thông báo ở notice board) là `display:inline-block` nên tiêu đề + "monthly:1-2 · nội dung" dính liền 1 dòng không xuống hàng. Đã ép `.notif-rule-row .notice-date { display:block }` (scoped riêng trong `<style>` của trang, không đụng rule `.notice-date` dùng chung). Đồng thời đổi bố cục sang 2 cột kiểu danh sách: danh sách nhắc định kỳ hiện có bên trái (mỗi dòng: tiêu đề + meta xuống dòng riêng, Bật/Sửa/Xoá gom phải), form thêm/sửa bên phải (`.recur-rule-layout`/`.recur-rule-list-card`/`.recur-rule-form-card`) — tự rớt xuống 1 cột khi màn hẹp.
- **Thẻ Team (`team.html`) bỏ badge Cấp bậc (Founder/Quản lý/Nhân viên), đổi sang hiện "Bộ phận · Phòng ban"** (VD "BO · BOD") ở đúng vị trí badge cũ trong `renderTeamGrid()` (`portal.js`) — người dùng thấy badge cấp bậc thừa vì chức danh (role) đã đủ nói lên vai trò. Suy ngược `divisionCode` từ `departmentCode` nếu thiếu (giống cách profile.html làm), không có dữ liệu Bộ phận/Phòng ban nào thì ẩn hẳn pill đó (không hiện rỗng). Hover vào pill hiện tooltip tên đầy đủ (`title` = "Bộ phận · Phòng ban" bằng tiếng Việt). CHỈ đổi ở thẻ lưới — modal chi tiết khi bấm vào thẻ vẫn giữ nguyên badge Cấp bậc + 2 dòng Bộ phận/Phòng ban riêng như cũ (không được yêu cầu đổi).

- **Service Worker cache khiến điện thoại không thấy code mới** (lỗi nghiêm trọng nhất phiên này) — xem chi tiết đầy đủ ở mục "Offline-first" trong QUY TẮC CỐ ĐỊNH phía trên (đã viết lại nguyên khối, đọc bản đó thay vì đoán). Tóm tắt: `sw.js` đổi "stale-while-revalidate" → "network-first + no-store" cho MỌI request, bump `CACHE_VERSION` lên `hiconique-shell-v2`, `offline.js` tự reload 1 lần khi SW mới giành quyền kiểm soát (`controllerchange`).
- **Offline.guard() chặn oan thao tác ghi dù mạng vẫn tốt** — nâng ngưỡng 3→10 lần ping trượt liên tiếp + chỉ chặn khi `navigator.onLine` cũng báo mất mạng. Xem mục Offline-first.
- **Có mạng lại tự reload cả trang, xoá mất form đang gõ dở** — đổi sang `TaskManager.silentRefresh()` (không reload). Xem mục Offline-first.
- **Trang tự reload liên tục gây nháy màn hình mỗi ~10-30s** — nguyên nhân gốc: bản trước dùng `reloadIfStale()` (reload cả trang) chạy ĐỊNH KỲ mỗi 15s, mỗi lần reload lại tự đặt lại mốc "vừa đồng bộ" nên ngay sau đó lại "cũ" và reload tiếp, lặp vô hạn. Đã bỏ hẳn reload định kỳ, thay bằng `silentRefresh()` (không reload) — CHỈ còn 1 tình huống thật sự reload cả trang: vừa mở lại app từ bfcache (đã bị đưa ra nền rồi mở lại, `pageshow` + `event.persisted`).
- **Dropdown Tỉnh/Thành (form Thêm dự án) thỉnh thoảng trống trơn** — trước đó fetch trực tiếp mỗi lần mở modal, không cache, 1 lần mạng chậm/lỗi là trống vĩnh viễn cho lần mở đó. Đã thêm cache `localStorage['hiconique_province_list']` — điền ngay từ cache trước, fetch mới chạy nền cập nhật. Áp dụng tương tự cho dropdown "Loại dự án" (`hiconique_building_type_list`).
- **Thành viên đang chờ duyệt/bị từ chối vẫn chọn được khi giao việc/thêm vào dự án** (người dùng test bằng 1 tài khoản "bot" tự đăng ký, thấy nó vẫn hiện trong danh sách chọn người ngay cả khi chưa duyệt) — thêm `TaskManager.getActiveMembers()` (chỉ status `active`/không có status), dùng ở MỌI nơi CHỌN người: `openProjectModal`/`openTaskModal` (`task-manager-app.js`), `renderProjectMembers`/`renderTaskAssigneeDropdown`/sidebar lọc theo người (`projects.js`). `TaskManager.getMembers()` (tất cả, kể cả chờ duyệt/bị từ chối) vẫn giữ nguyên cho trang Team (`portal.js`) — nơi DUY NHẤT cần thấy đủ mọi trạng thái để CEO/Manager duyệt.
- **Nút "Đánh dấu đã đọc tất cả" (chuông thông báo) không có phản hồi gì** — bug thiếu gọi `updateBadge()` sau khi đánh dấu, chấm đỏ trên chuông không bao giờ tắt khiến người dùng tưởng nút hỏng dù dữ liệu đã lưu đúng. Đã fix + làm nền thông báo CHƯA ĐỌC đậm rõ hơn (14% opacity + viền trái đồng, trước chỉ 6% gần như vô hình) + thêm nút "Xoá tất cả thông báo" cuối panel (dưới "Nhắc định kỳ hiện có") — CHỈ ẩn khỏi danh sách của CHÍNH người bấm trên MÁY đó (`localStorage['hiconique_dismissed_notifications']`, không đồng bộ Sheet), nên ai cũng bấm được không cần quyền quản lý.
- **Kéo xuống đầu trang (mobile) để reload thủ công** — tính năng mới, xem mục "Offline-first" điểm 5 phía trên.
- **Duyệt/Từ chối đăng ký + tự động xoá tài khoản bị Từ chối sau 48h**: chỉ Founder/CEO/Giám đốc Bộ phận (`roleLevel==='admin'`, xem `TaskManager.canManageMembers()` trong `task-data.js` — đã THU HẸP từ "admin hoặc manager" xuống chỉ còn admin) mới duyệt/từ chối được người đăng ký mới, qua nút trong modal team (`portal.js` `openTeamMemberModal()`). Từ chối (qua nút web HOẶC tự sửa tay cột "Trạng thái" thẳng trên Sheet thành "Từ chối") đều kích hoạt CÙNG 1 cơ chế:
  - Server (`gsheets-api-v2.js`) tự stamp cột mới "Thời điểm từ chối" (field `rejectedAt`, chỉ stamp 1 LẦN — từ chối lại lần 2 mà chưa từng duyệt lại giữa 2 lần không ghi đè mốc cũ), điền thêm 2 cột hiển thị "Sẽ xoá lúc" (ngày giờ tuyệt đối) và "Đếm ngược" (công thức Sheet tự trừ `NOW()`, cập nhật mỗi phút — đã bật `setRecalculationInterval(MINUTE)`), và gửi thông báo nội bộ cho Founder (ưu tiên đúng `level==='founder'`, fallback mọi `roleLevel==='admin'` nếu chưa gán ai làm Founder).
  - Duyệt lại (đổi status ra khỏi 'rejected') TRƯỚC KHI hết 48h sẽ tự xoá mốc `rejectedAt` — huỷ hẳn lịch xoá, lần từ chối sau (nếu có) tính lại đủ 48h mới từ đầu.
  - Trigger định kỳ 30 phút (`deleteExpiredRejectedMembers` + `setupAutoDeleteRejectedMembersTrigger`, đã cài) quét mọi dòng status HIỆN TẠI vẫn là 'rejected' và `rejectedAt` quá 48h → xoá HẲN dòng đó khỏi Sheet (vĩnh viễn, không khôi phục được).
  - Team card/modal (`portal.js`) hiện đồng hồ đếm ngược SỐNG (tự cập nhật mỗi giây bằng `setInterval`, không cần reload) cho tài khoản đang ở trạng thái Từ chối — tính `deleteAt = rejectedAt + 48h` ngay trên client, không cần đọc thêm field nào qua API.
  - **Bẫy đã gặp + đã sửa**: công thức "Đếm ngược" set qua `Range.setFormula()` từ Apps Script VẪN bị áp dụng theo locale của Sheet (bảng tính này ở locale VN, cần dấu `;` phân cách đối số thay vì `,`, giống hệt khi gõ tay công thức trực tiếp — nhà phát triển trước đây từng nghĩ điều này chỉ áp dụng khi gõ tay, KHÔNG áp dụng cho `setFormula()`, thực tế không đúng). Dùng dấu `,` bị báo "Lỗi phân tích cú pháp công thức" ngay — đã sửa dùng `;`, và có hàm `fixRejectionCountdownFormulas()` để vá lại công thức cho các dòng đã bị lỗi từ trước khi sửa.
  - Đã thay `window.confirm()`/`alert()` gốc trong luồng duyệt/từ chối (portal.js) bằng `showConfirmDialog()`/`showToast()` tự dựng (đã có sẵn pattern y hệt trong `timesheet.html`, copy CSS `.ts-confirm-*` sang `portal.css`) — phát hiện lại đúng bug đã chốt cấm 2026-09-15 (dialog native treo cứng tab khi test tự động hoá) trong lúc test tính năng này.
  - Đã deploy Apps Script phiên bản 65, test đầy đủ trên trình duyệt thật: nút Từ chối → đồng hồ đếm ngược hiện đúng ngay lập tức cả trên web lẫn trên Sheet, thông báo gửi đúng tới Founder, công thức Đếm ngược không còn lỗi.

- **Fix: app "Thêm vào Màn hình chính" (PWA) không tự cập nhật dữ liệu, phải đăng xuất/đăng nhập lại** — nguyên nhân: `task-data.js` chỉ `initData()` MỘT LẦN lúc trang load, không có cơ chế tự làm mới định kỳ; trên máy tính hay vô tình fetch lại do chuyển trang, nhưng app cài vào màn hình chính (standalone, iOS Safari/Android Chrome) khi chuyển về nền rồi mở lại thường bị trình duyệt phục hồi từ bfcache — KHÔNG chạy lại JS, DOM đứng yên với dữ liệu cũ. Đã thêm `reloadIfStale()` trong `offline.js`: tự động `window.location.reload()` khi dữ liệu cũ quá 20 giây VÀ trang đang thực sự hiển thị (không reload ngầm lúc app ở nền) — bắt qua sự kiện `pageshow` (cờ `event.persisted`, điểm mấu chốt bắt đúng ca bfcache) + `visibilitychange` + vòng lặp ping 15s có sẵn. Đã test bằng cách giả lập stale + visible + monkeypatch `window.location.reload()` trên trình duyệt — xác nhận trigger đúng.

- **Fix nhỏ đăng ký/Giới tính**: form đăng ký (modal `auth.js`) — "Bộ phận"/"Phòng ban" tách khỏi grid 2 cột, mỗi field giờ full-width riêng 1 dòng (trước đó text dài như "Khối Kỹ thuật Xây dựng & Sản xuất" bị cắt do chung dòng với ô kia, đã test cả desktop lẫn mobile). Bỏ hẳn lựa chọn "Khác" ở Giới tính (đăng ký + trang cá nhân), chỉ còn Nam/Nữ. Thêm `VALUE_MAP` cho `members.gender` (Nam/Nữ ↔ male/female) — Sheet trước đó hiện "male"/"female" tiếng Anh thô do cột này chưa từng được Việt hoá; đã chạy `normalizeMemberGender()` 1 lần để chuẩn hoá dữ liệu cũ.

- **Phân tầng Cấp bậc (Level) — 5 mức, thay thế `roleLevel` 3 mức cũ làm nguồn dữ liệu chính**: cột Sheet "Cấp bậc" (NS-Thành viên) đổi field nội bộ từ `roleLevel` (admin/manager/member) sang `level` (`founder`/`ceo`/`dept_director`/`manager`/`member`, nhãn Việt "Founder"/"CEO"/"Giám đốc Bộ phận"/"Quản lý"/"Nhân viên" — xem `LEVELS` trong `public/js/task-data.js` và `gsheets-api-v2.js`). **Không refactor ~25 chỗ check `roleLevel==='admin'/'manager'` rải rác trong client** — thay vào đó `getAllData()` (Apps Script) TỰ SUY `roleLevel` từ `level` mỗi lần đọc (`LEVEL_TO_ROLELEVEL`: founder/ceo/dept_director → admin, manager → manager, member → member) nên toàn bộ code permission cũ chạy y nguyên không cần sửa. `auth.js` đăng ký mới luôn gán `level: 'member'` (mức thấp nhất, admin tự nâng tay qua Sheet sau). Badge cấp bậc ở team card/modal (`portal.js`) và trang cá nhân (`profile.html`) ưu tiên đọc `user.level` qua `TaskManager.getLevelByCode()`, có fallback về `roleLevel` cũ nếu `level` trống (dữ liệu rất cũ).
  - Dữ liệu cột "Cấp bậc" trên Sheet từng bị lẫn lộn tiếng Anh thô ("CEO"/"manager"/"member") và tiếng Việt đã chuẩn hoá — đã chạy `normalizeMemberLevels()` 1 lần để đồng nhất hết về nhãn Việt chuẩn (CEO/Quản lý/Nhân viên), xác nhận qua log + xem trực tiếp trên Sheet.
  - **Giới hạn đã xác nhận (KHÔNG cố sửa tiếp, tốn nhiều công mà rủi ro mất dữ liệu cao)**: Google Sheets chặn ghi đè data-validation lên Ô ĐÃ CÓ GIÁ TRỊ ("Thao tác này không được phép ở các ô trong các cột đã nhập"). Thử né bằng cách xoá tạm giá trị → đặt validation mới → ghi lại giá trị CÓ GÂY MẤT DỮ LIỆU THẬT (SpreadsheetApp gộp lệnh ghi, chỉ commit khi script kết thúc — nếu bước cuối lỗi thì các ghi ĐÃ LOG THÀNH CÔNG vẫn có thể bị mất, đã xảy ra và phải khôi phục thủ công từ dữ liệu đã xác nhận trước đó). Vì vậy `applyLevelDropdown()` (trong `gsheets-api-v2.js`) CHỈ áp dropdown 5 mức mới lên các DÒNG TRỐNG phía dưới dữ liệu hiện có (dòng nhập tay mới sau này) — các dòng dữ liệu cũ (2 trở đi tại thời điểm chạy) giữ nguyên dropdown UI kiểu cũ (3 mức, tự vẫn đọc/ghi đúng qua VALUE_MAP dù giao diện dropdown còn cũ). Nếu sau này cần dropdown 5 mức TRÊN CHÍNH các ô dữ liệu cũ, phải xoá tay data-validation của range đó qua UI Sheets trước (Dữ liệu → Xác thực dữ liệu → Xoá xác thực) rồi mới chạy lại `applyLevelDropdown()`.
  - Apps Script đã deploy phiên bản 62 (đúng deployment production khớp `API_URL` trong `gsheets-config.js`) — đã xác minh `?action=getMembers` trả về cả `level` và `roleLevel` suy đúng cho từng thành viên, và đã test trên trình duyệt (badge team card/modal + trang cá nhân đều hiển thị đúng nhãn theo `level` mới).

- **Offline-first** (mới thêm, xem quy tắc cố định "Offline-first" ở trên để hiểu kiến trúc) — `public/sw.js` (Service Worker precache app shell) + `public/js/offline.js` (banner mất mạng kèm mốc giờ dữ liệu, tự reload khi có mạng lại, chặn toàn bộ thao tác ghi khi offline) đã nạp vào cả 19 trang. Đã test được trên trình duyệt (giả lập mất mạng qua `window.dispatchEvent(new Event('offline'))` + chặn `fetch`): banner hiện đúng chữ + giờ, `TaskManager.createTask()` bị chặn hoàn toàn (không ghi localStorage, không gọi API, hiện toast đỏ), có mạng lại tự reload. **CHƯA test được** phần Service Worker thật sự cache app-shell để mở lúc KHÔNG CÓ mạng — môi trường trình duyệt xem trước (Claude Browser pane) tự chặn `navigator.serviceWorker.register()` (lỗi "An unknown error occurred when fetching the script" ngay cả với 1 SW rỗng tối giản — đã xác minh đây là giới hạn của sandbox xem trước, không phải lỗi code, vì cùng file fetch thường vẫn 200 OK bình thường). **Cần người dùng tự test thật** trên điện thoại (Safari iOS/Chrome Android) sau khi deploy: mở app 1 lần có mạng trước (để Service Worker cài đặt xong), sau đó bật chế độ máy bay rồi mở lại app xem có load được không.

- **Đổi logo công ty** — thay icon "Thêm vào Màn hình chính" (trước đó không có, iOS tự chụp screenshot trang làm icon xấu + tên bị cắt "HICONIQUE·Inte...") bằng bộ icon PNG nền gradient đen→xám→xanh + logo trắng, và thay TOÀN BỘ chữ "H" (logo header 19 trang, login.html, modal đăng nhập/đăng ký) bằng logo skyline chính thức mới — xem quy tắc cố định "Logo chính thức của công ty" ở đầu file.
- **Bộ phận (cấp CHA của Phòng ban)** — thêm field `division`/`divisionCode` cho Thành viên (2 cột mới "Bộ phận"/"Mã bộ phận" trên Sheet "NS-Thành viên"), danh mục CỐ ĐỊNH 4 khối theo SOP (`TaskManager.getDivisions()`/`getDivisionByCode()`/`getDepartmentsByDivision()` trong `task-data.js`; mỗi `DEPARTMENTS[i].divisionCode` trỏ về đúng 1 Bộ phận cha). Mã: BO (Back-Office), FO (Front-Office), DDC (Design & Data Core), CPC (Construct & Product Core) — lấy từ tên tiếng Anh trong ngoặc của SOP, cùng quy ước với mã Phòng ban. Form đăng ký (`auth.js`) và trang cá nhân (`profile.html`) có 2 dropdown CHA/CON: chọn Bộ phận trước sẽ tự lọc Phòng ban chỉ còn các phòng thuộc khối đó. Thành viên đã có Phòng ban TỪ TRƯỚC (gán khi chưa có field Bộ phận) được tự suy ngược ra Bộ phận qua `departmentCode` khi hiển thị/sửa (không cần gán tay lại) — xem `profile.html` render()/`portal.js` openTeamMemberModal(). Apps Script: `FIELD_MAP.members` thêm `['Bộ phận', 'division'], ['Mã bộ phận', 'divisionCode']` + hàm migrate 1 lần `addDivisionColumns()` (đã chạy), cùng `applyDepartmentDropdowns()` đặt dropdown data-validation CỐ ĐỊNH cho cả 4 cột Bộ phận/Mã bộ phận/Phòng ban/Mã phòng ban trên Sheet (không gõ tay tự do được nữa) — DIVISION_NAMES/DEPARTMENT_NAMES... trong `gsheets-api-v2.js` PHẢI khớp đúng danh sách trong `task-data.js`, đổi 1 bên phải đổi bên kia. Deploy phiên bản 59, đã test đủ trên trình duyệt thật (dropdown cha/con ở form đăng ký, sửa+lưu ở trang cá nhân, hiển thị đúng ở thẻ/modal team, xác nhận round-trip qua API `getMembers`).
- **Phòng ban** — thêm field `department`/`departmentCode` cho Thành viên (2 cột mới trên Sheet "NS-Thành viên": "Phòng ban"/"Mã phòng ban"), danh mục CỐ ĐỊNH **19** phòng ban theo SOP nội bộ (`TaskManager.getDepartments()`/`getDepartmentByCode()` trong `task-data.js` — 1 nguồn duy nhất, KHÔNG tự thêm/sửa danh sách này khi không có yêu cầu). Áp dụng ở form đăng ký (`auth.js`), trang cá nhân tự sửa (`profile.html`), thẻ + modal chi tiết thành viên (`portal.js` — `renderTeamGrid()`/`openTeamMemberModal()`). Apps Script: `FIELD_MAP.members` thêm `['Phòng ban', 'department'], ['Mã phòng ban', 'departmentCode']` + hàm migrate 1 lần `addDepartmentColumns()` (đã chạy xong, có thể xoá khỏi code sau). Deploy phiên bản 58, đã test đủ trên trình duyệt thật (đăng ký/sửa/hiển thị thẻ+modal team, xác nhận round-trip qua API `getMembers`).
  - **Cập nhật 2026-09-18**: đồng bộ lại đúng theo SOP mới nhất — thêm phòng ban **DRW** (Kỹ thuật Triển khai 2D), đổi mã **EST → QS** (Dự toán & Bóc tách) và **QAS → QAC** (Quản lý Chất lượng, đổi cả tên), đủ 19 phòng ban. Đã kiểm tra không có thành viên nào đang gán mã EST/QAS trước khi đổi (an toàn, không cần migrate dữ liệu cũ). Thêm field `desc` (mô tả chức năng) cho mỗi Bộ phận (`DIVISIONS`) và Phòng ban (`DEPARTMENTS`) trong `task-data.js`, dùng để hiện tooltip (`title="..."`) khi di chuột vào dòng Bộ phận/Phòng ban ở modal chi tiết thành viên (`portal.js` — `openTeamMemberModal()`). **CẦN LÀM THỦ CÔNG**: đã sửa `DIVISION_NAMES`/`DEPARTMENT_NAMES`/`DEPARTMENT_CODES` trong `gsheets-api-v2.js` cho khớp, nhưng CHƯA paste vào Apps Script editor + redeploy + chạy lại `applyDepartmentDropdowns()` — tới lúc đó dropdown "Phòng ban"/"Mã phòng ban" trên Sheet vẫn còn hiện danh sách cũ (18 mục, có EST/QAS, thiếu DRW).
- **Chọn nhanh tháng/năm ở Báo cáo chấm công** (`timesheet.html`) — bấm vào nhãn "Tháng N/YYYY" trong modal "Xem báo cáo chấm công" mở lưới 12 tháng + nút chuyển năm, giống hệt pattern đã có ở Timeline/Gantt (`projects.js`). Đã test trên trình duyệt thật, hoạt động đúng.

- Duyệt thiết bị chấm công (CEO/Manager chấp nhận/từ chối thiết bị mới của nhân viên) + panel "Thiết bị chấm công chờ duyệt" trong trang Chấm công — ĐÃ deploy + test đủ luồng đăng ký/duyệt/từ chối/gỡ trên trình duyệt thật. Xem mục "Trạng thái hiện tại — 2026-09-15 (d)" ngay dưới.
- Nút "Quản lý địa điểm GPS/IP" (thêm/sửa/xoá) ngay trong trang Chấm công — ĐÃ deploy + test đủ trên trình duyệt thật. Xem mục "Trạng thái hiện tại — 2026-09-15 (a)/(b)/(c)" ngay dưới.
...
## Trạng thái hiện tại (cập nhật lần cuối: 2026-09-15 (d) — duyệt thiết bị chấm công + nút quản lý địa điểm GPS/IP, Apps Script phiên bản 54, ĐÃ XÁC NHẬN TRÊN TRÌNH DUYỆT)

**Yêu cầu người dùng**: (1) thêm nút "Quản lý địa điểm GPS/IP" (thêm/sửa/xoá) ngay trong trang Chấm công thay vì phải sửa qua Sheet, CHỈ CEO thấy/dùng được. (2) Khi nhân viên gỡ thiết bị chấm công → báo CEO. (3) Thiết bị MỚI đăng ký phải chờ CEO duyệt mới dùng được để chấm công, có note thể hiện đã duyệt/chưa trong trang Chấm công của CEO, và cũng hiện ở mục Thông báo.

- **Nút "Quản lý địa điểm GPS/IP"** (`timesheet.html`, cạnh nút "Xuất báo cáo chấm công", CHỈ admin thấy) — mở modal (dùng lại `.modal-overlay`/`.form-*` có sẵn từ `task-manager.css`) liệt kê + thêm/sửa/xoá từng dòng trong sheet "Địa điểm chấm công", ghi thẳng qua `TaskManager.addAttendanceLocation()/updateAttendanceLocation()/deleteAttendanceLocation()` (mới, trong `task-data.js`) — không cần vào tay Google Sheet nữa.
- **Duyệt thiết bị chấm công**: `deviceIds` của Thành viên đổi định dạng từ `"id::tên"` sang `"id::tên::status"` (status: `pending`/`approved`/`rejected`, thiếu status = coi như `approved` để không khoá thiết bị cũ đang hoạt động của người dùng hiện tại). `registerMemberDevice()` (`task-data.js`): thiết bị MỚI của nhân viên thường luôn `pending`; CEO/Manager tự đăng ký thiết bị của CHÍNH mình thì auto-`approved` luôn (không lẽ tự đăng ký xong phải tự chờ chính mình duyệt). `checkDeviceStatus()` (`timesheet.html`): chỉ tính "Thiết bị quen dùng" (1/3 điều kiện chấm công) khi status = `approved`; `pending` hiện "Thiết bị đang chờ CEO/Quản lý duyệt" và KHÔNG cho qua điều kiện đó.
- **Panel "Thiết bị chấm công chờ duyệt"** (`timesheet.html`, full-width dưới `.ts-grid`, CHỈ admin/manager thấy) — gom `TaskManager.getPendingDeviceRegistrations()` (thiết bị `pending` của TOÀN CÔNG TY, không chỉ của người đang đăng nhập), mỗi dòng có nút Duyệt/Từ chối gọi `approveMemberDevice()`/`rejectMemberDevice()` (mới) — đây chính là "note thể hiện có chấp nhận đồng ý hay không" người dùng yêu cầu. Từ chối = XOÁ hẳn entry (nhả lại slot trong tối đa 2 thiết bị/người), không giữ trạng thái `rejected`.
- **Thông báo hệ thống**: đăng ký thiết bị mới → báo mọi CEO/Manager (trừ chính người đăng ký nếu họ là CEO/Manager); gỡ thiết bị → báo mọi CEO/Manager; duyệt/từ chối → báo riêng người sở hữu thiết bị đó. Dùng hàm mới `TaskManager.addSystemNotificationsBatch()` — KHÔNG qua `canManageNotifications()` như `createNotification()` (nhân viên thường vẫn phải tự kích hoạt được loại thông báo này khi tự đăng ký/gỡ thiết bị của chính họ — đây là sự kiện hệ thống ghi hộ, không phải nội dung tự soạn).
- **Bẫy nghiêm trọng gặp khi test thật (2026-09-15)**: lúc đầu gửi thông báo bằng vòng lặp gọi `addSystemNotification()` (đơn lẻ) cho từng CEO/Manager — gửi cho 4 người, đọc lại chỉ còn 3 thông báo, 2 trong số đó còn TRÙNG LUÔN CẢ ID (đúng lớp bug "gọi API song song trong vòng lặp" đã cảnh báo sẵn trong comment của `addDataBatch()` cho vụ seed Tiến độ trước đây, nay tái diễn ở chỗ khác). **Đã sửa triệt để**: thêm action `addNotificationsBatch` (server, gọi `addDataBatch()` — 1 lệnh `setValues()` duy nhất, không có 2 lệnh nào chạy chồng nhau) + đổi `addSystemNotification()` thành `addSystemNotificationsBatch(dataList)` (LUÔN gửi theo mảng, kể cả 1 người nhận) — đã test lại gửi 4 thông báo cùng lúc qua batch mới, đủ 4/4, không trùng ID. Xem quy tắc mới thêm ở đầu file.
- **Bẫy nghiêm trọng khác gặp khi test qua Claude in Chrome**: nút "Gỡ" thiết bị và validate form địa điểm dùng `confirm()`/`alert()` gốc — cả 2 lần đều làm TREO CỨNG tab thật (dialog native chặn main thread, không chụp màn hình/không click được nữa, phải đóng hẳn tab). **Đã thay bằng `showConfirmDialog()`/`showToast()` tự dựng** (không chặn) — xem quy tắc mới thêm ở đầu file, áp dụng cho MỌI xác nhận/thông báo mới thêm sau này trong toàn bộ web, không riêng trang Chấm công.
- **Đã test đủ trên trình duyệt thật (2 tài khoản thật)**: đăng nhập nhân viên thường (Giản Phương) → đăng ký thiết bị → status `pending`, CEO nhận đúng thông báo; đăng nhập CEO (Phạm Quang Hiếu) → panel hiện đúng "1 đang chờ" → bấm Duyệt → nhân viên nhận thông báo đã duyệt, badge đổi "Đã duyệt"; quay lại tài khoản nhân viên bấm "Gỡ" → dialog tự dựng hoạt động đúng, không treo, CEO nhận đủ thông báo gỡ thiết bị (4/4, không mất).

## Trạng thái hiện tại (cập nhật lần cuối: 2026-09-15 (c) — Apps Script phiên bản 53, bug IP bị Sheet tự đọc nhầm thành số, ĐÃ XÁC NHẬN TRÊN TRÌNH DUYỆT)

**Bẫy nghiêm trọng phát hiện khi verify mục (a) bên dưới**: sau khi seed 3 dòng vào sheet "Địa điểm chấm công", đọc lại qua API thấy IP `14.171.113.174` bị Sheet tự đổi thành SỐ `14171113174` (mất hết dấu chấm) — trong khi IP còn lại `172.225.56.21` vẫn đúng dạng chuỗi. Lý do: Sheet ở locale Việt Nam đọc `.` là dấu phân cách hàng nghìn; IP nào có các cụm sau octet đầu ĐÚNG 3 chữ số (như `14.171.113.174`) trông y hệt 1 số được nhóm hàng nghìn hợp lệ nên bị tự nhận thành số, còn IP có cụm không đủ 3 chữ số (như `172.225.56.21`, cụm `56`/`21`) thì tình cờ thoát nạn — khiến bug rất dễ lọt qua nếu chỉ test 1 IP.

- **Kỹ thuật ép TEXT cũ (`"'" + val` — dấu nháy đơn ở đầu chuỗi) ĐÃ KIỂM CHỨNG LÀ KHÔNG ĐÁNG TIN CẬY khi gọi qua `Range.setValue()` từ Apps Script API** (dù đây là quy ước quen thuộc khi gõ tay trên UI Sheets) — thử ép lại IP bằng cách này vẫn ra số y hệt. Đã đổi hẳn sang cách CHẮC CHẮN: đặt `range.setNumberFormat('@')` (Văn bản thuần) cho đúng ô đó TRƯỚC khi ghi giá trị — xem `needsPlainTextFormat()`/`writeTextForcedCell()` mới trong `gsheets-api-v2.js`, áp dụng thống nhất cho mọi field `ip`/`*Ip` VÀ 3 field cũ `phone`/`cccd`/`bankAccount` (cùng lớp bug, chỉ chưa bị phát hiện vì hiếm khi trùng đúng cách nhóm 3 chữ số).
- `addData()`/`updateData()` đều sửa: field cần ép text để trống lúc `appendRow`/`setValue` đầu tiên, rồi ghi lại riêng 1 lần nữa sau khi đã đặt định dạng ô — đảm bảo Sheet không kịp tự nhận diện sai trước khi giá trị text "chốt" vào ô.
- Đã deploy Apps Script phiên bản 53 (bản có fix này) và tự sửa lại dòng dữ liệu bị hỏng qua `updateAttendanceLocation` — xác nhận lại bằng API (trả về đúng `"14.171.113.174"` dạng chuỗi) VÀ bằng mắt trên Sheet thật (ô căn trái = kiểu Text, không còn căn phải như số).
- **Đã mở trang Chấm công thật trên trình duyệt (Claude in Chrome) — panel "Vị trí GPS" hiển thị đúng "Đang ở gần Văn phòng — 167 Trường Chinh, Lê Thanh Nghị, Hải Phòng"**, xác nhận `checkGeoStatus()` đọc đúng từ sheet mới và hiển thị đúng tên địa điểm.

## Trạng thái hiện tại (cập nhật lần cuối: 2026-09-15 (b) — fix "Tasks của tôi" ở Dashboard hiện tràn/hiện việc cả công ty, ĐÃ XÁC NHẬN TRÊN TRÌNH DUYỆT)
...
- **ĐÃ XÁC NHẬN trên trình duyệt thật** (`#panel-tasks`, tài khoản Phạm Quang Hiếu `CEO_QH_030800`) — panel hiện đúng "Việc của tôi: 0" (kiểm tra chéo qua console: tài khoản này thật sự không có task nào trong 91 task của công ty khớp `assigneeIds`, không phải bug), không còn tràn trang.

## Trạng thái hiện tại (cập nhật lần cuối: 2026-09-15 (a) — sheet "Địa điểm chấm công" (GPS/IP không giới hạn số lượng), ĐÃ DEPLOY + XÁC NHẬN — xem bug IP bị đọc nhầm thành số ở mục (c) phía trên)
...
- **Đã deploy Apps Script (phiên bản 53) + chạy `seedAttendanceLocations()` thành công** (seed 3 dòng vào sheet đặt ngay cạnh "Chấm công") **+ xác nhận trên trình duyệt thật** — trang Chấm công đọc đúng GPS/IP từ sheet mới (xem chi tiết + bug IP đã gặp ở mục (c) phía trên).
