...
**Phiên 2026-09-23 — Thông báo đẩy (Web Push qua Firebase) — CHƯA XONG HẲN, còn 1 bước THỦ CÔNG bắt buộc trước khi hoạt động được, xem mục ngay dưới.** Apps Script đã redeploy **phiên bản 78** (thêm `pushForNotificationRow_()`/`sendPushToMember_()`/`getFcmAccessToken_()` + sheet `TT-Thiết bị đăng ký thông báo` + action `registerPushDevice`/`unregisterPushDevice`).

### Phiên 2026-09-23 — Thông báo đẩy (Web Push) — popup thật trên điện thoại/máy tính giống Zalo

- **Kiến trúc**: dùng Firebase Cloud Messaging (FCM), KHÔNG tự viết crypto Web Push trong Apps Script (quá phức tạp/dễ lỗi vặt, xem lý do đã trao đổi với người dùng lúc chọn hướng làm). Apps Script gửi push bằng cách tự làm luồng OAuth2 service-account (ký JWT RS256 qua `Utilities.computeRsaSha256Signature` có sẵn trong Apps Script) rồi gọi thẳng REST API `fcm.googleapis.com/v1/.../messages:send` — không cần thư viện ngoài nào.
- **Project Firebase**: tên "HICONIQUE Internal Hub", project ID **`hiconique-internal-hub-f77f2`**, tạo dưới tài khoản Google **hiconique.group@gmail.com** (**LƯU Ý: lần đầu tạo nhầm dưới tài khoản cá nhân pqhieu3820@gmail.com — project đó (`hiconique-internal-hub`, không có hậu tố) vẫn còn tồn tại trên Gmail cá nhân, CHƯA XOÁ, không dùng nữa — có thể vào Firebase Console tài khoản đó xoá đi cho gọn bất cứ lúc nào, không ảnh hưởng gì tới project thật đang dùng**). Gói Spark (miễn phí), tắt Gemini/Analytics lúc tạo (không cần cho việc gửi push).
- **File đụng tới**:
  - `gsheets-api-v2.js`: sheet mới `TT-Thiết bị đăng ký thông báo` (`SHEETS.pushDevices`, cột `id/memberId/fcmToken/deviceLabel/browserFamily/active/createdAt/lastActiveAt`); action `registerPushDevice` (upsert theo `fcmToken`)/`unregisterPushDevice`; hàm `getFcmAccessToken_()`/`sendPushToMember_()`/`pushForNotificationRow_()` cuối file — móc DUY NHẤT vào `addData()`/`addDataBatch()` khi `sheetName === SHEETS.notifications`, nên MỌI đường tạo thông báo (client gọi `addNotification(sBatch)` HAY server tự ghi thẳng như `autoCheckoutForgottenEntries`/`notifyFounderMemberRejected`...) đều tự động kích hoạt gửi push, không phải sửa từng nơi.
...
- **1 BƯỚC THỦ CÔNG BẮT BUỘC (chưa xong tính tới cuối phiên này)**: dán nội dung file JSON service account (đã tải 2 lần — 1 lần nhầm tài khoản cá nhân, 1 lần đúng tài khoản `hiconique.group@gmail.com`, DÙNG FILE CỦA LẦN ĐÚNG TÀI KHOẢN) vào Script Property `FCM_SERVICE_ACCOUNT_JSON` trong Apps Script (Cài đặt dự án > Thuộc tính của tập lệnh — đã gõ sẵn tên thuộc tính, chỉ còn thiếu dán giá trị + bấm Lưu). **Lý do phải để người dùng tự làm**: hệ thống an toàn tự động chặn AI đọc/dò file credential vừa tải xuống máy (phân loại "Credential Materialization/Exploration") dù là do chính phiên làm việc tạo ra — đây là giới hạn cố định, không phải lỗi, không tìm cách lách qua bước này lần sau, luôn nhờ người dùng tự dán.
- **Chưa test round-trip thật** (do đang chờ bước thủ công ở trên) — sau khi người dùng xác nhận đã lưu `FCM_SERVICE_ACCOUNT_JSON`, việc tiếp theo: bấm "Bật thông báo" thử ở 1 trang, xác nhận token được ghi vào sheet `TT-Thiết bị đăng ký thông báo`, tạo 1 thông báo test (VD tự giao task cho mình) xem có popup thật hiện ra không, rồi dọn sạch dữ liệu test.
