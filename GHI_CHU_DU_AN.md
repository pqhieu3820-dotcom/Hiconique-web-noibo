...
Không có việc gì đang dở dang — mọi thay đổi trong phiên này đều đã deploy + xác nhận trên trình duyệt (chờ lệnh push cuối phiên).

- Sheet "Địa điểm chấm công" (GPS + IP không giới hạn số lượng) + fix "Tasks của tôi" tràn/hiện việc cả công ty — ĐÃ deploy (Apps Script phiên bản 53) + xác nhận hoạt động đúng trên trình duyệt thật. Xem mục "Trạng thái hiện tại — 2026-09-15 (a)/(b)/(c)" ngay dưới.
